...
public class Module implements MinecraftInstance {
    ...
    public void toggle(ToggleType type) {
        ...
        boolean toggleNotifications = Lecture.INSTANCE.getModuleManager().getModule(HUD.class).toggleNotifications.isEnabled();
        if (this.enabled) {
            this.onEnable();
            Lecture.INSTANCE.getEventManager().register(this);
            try {
            ...
            if (this instanceof Scaffold) {
                ((Scaffold) this).anim.setDirection(Direction.BACKWARDS);
                Multithreading.schedule(() -> Lecture.INSTANCE.getEventManager().unregister(this), 0, TimeUnit.MILLISECONDS);
            } else {
                Lecture.INSTANCE.getEventManager().unregister(this);
            }
            try {
                if (toggleNotifications && type == ToggleType.MANUAL && !Objects.equals(this.name, "ClickGUI")) {
    ...
    public Module get(Class<? extends Module> module) {
        return Lecture.INSTANCE.getModuleManager().get(module);
    }

    public boolean isModuleEnabled(Class<? extends Module> module) {
        return Lecture.INSTANCE.getModuleManager().get(module).isEnabled();
    }

    public void toggle(Class<? extends Module> module) {
        Lecture.INSTANCE.getModuleManager().get(module).toggle(ToggleType.MANUAL);
    }
