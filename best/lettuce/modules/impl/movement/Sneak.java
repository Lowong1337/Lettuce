...
public class Sneak extends Module {
    ...
    public final EventListener<EventPacketSend> onSend = event -> {
        ...
        if (packet instanceof C0BPacketEntityAction) {
            if (((C0BPacketEntityAction) packet).getAction().equals(C0BPacketEntityAction.Action.START_SNEAKING) && modes.is("Vanilla") && !snuck) {
                snuck = true;
            } else if (((C0BPacketEntityAction) packet).getAction().equals(C0BPacketEntityAction.Action.START_SNEAKING) || ((C0BPacketEntityAction) packet).getAction().equals(C0BPacketEntityAction.Action.STOP_SNEAKING)) {
                event.cancel();
            }
        }
    };
