...
//                    }
                    String name = (formattedTags.isEnabled() ? entity.getDisplayName().getFormattedText() : StringUtils.stripControlCodes(entity.getDisplayName().getUnformattedText())) + ircName;
                    StringBuilder text = new StringBuilder((
                            //FriendCommand.isFriend(renderingEntity.getName()) ? "§d" :
                            "§f") + name);
                    if (healthText.isEnabled("Nametag")) {
                        text.append(String.format(" §7[§r%s HP§7]", df.format(((EntityLivingBase) entity).getHealth())));
                    }
    ...
    public final EventListener<EventRender2D> onRender = e -> {
        ...
        if (boxColorMode.is("Custom")) {
            switch (boxColorModes.getMode()) {
                ...
                case "Double Colors": gradientColorWheel(new Color[]{color1.getColor(), color2.getColor()}); break;
                case "Fade": gradientColorWheel(new Color[]{color1.getColor(), color1.getColor().darker().darker()}); break;
                case "Static": gradientColorWheel(new Color[]{color1.getColor(), color1.getColor()}); break;
            }
        } else {
            switch (hud.colorMode.getMode()) {
                ...
                case "Double Colors":
                    gradientColorWheel(new Color[]{hud.color1.getColor(), hud.color2.getColor()});
                case "Fade":
                    gradientColorWheel(new Color[]{hud.color1.getColor(), hud.color1.getColor().darker().darker()});
                    break;
                case "Static": gradientColorWheel(new Color[]{hud.color1.getColor(), hud.color1.getColor()}); break;
            }
        }
...
//                        ircName = " \2477(\247d" + IRCUtil.usersMap.get(renderingEntity.getName()) + "\2477)";
//                    }
                    float healthValue = ((EntityLivingBase) entity).getHealth() / ((EntityLivingBase) entity).getMaxHealth();
                ...
                if (esp.isEnabled("Held Item")) {
                    if (((EntityLivingBase) entity).getHeldItem() != null) {
                        ...
                        float middle = x + ((right - x) / 2);
                        float textWidth;
                        String text = ((EntityLivingBase) entity).getHeldItem().getDisplayName();
                ...
                if (esp.isEnabled("Armor")) {
                    ...
                    for (int i = 3; i >= 0; i--) {
                        if (((EntityLivingBase) entity).getCurrentArmor(i) == null) {
                            seperation += (length / 3) / scale;
                            continue;
                        }
                        mc.getRenderItem().renderItemAndEffectIntoGUI(((EntityLivingBase) entity).getCurrentArmor(i), (int) equipmentX, (int) (equipmentY + seperation));
                        seperation += (length / 3) / scale;
                    }
