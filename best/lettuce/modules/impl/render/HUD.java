...
public class HUD extends Module {
    ...
    public final StringProperty watermark = new StringProperty("Watermark Text", "Lecture Client", () -> (hudOptions.isEnabled("Watermark") && !watermarkMode.is("Logo")));
    ...
    public void getModulesAndSort() {
        if (modules == null || ModuleManager.reloadModules) {
            modules = Lecture.INSTANCE.getModuleManager().getModules();
        }
    ...
    public void drawWatermark() {
        ...
        switch (watermarkMode.getMode()) {
            ...
            case "Khoi": {
                getFont().drawString(EnumChatFormatting.WHITE + text + EnumChatFormatting.WHITE + " (" + EnumChatFormatting.RESET + Lecture.VERSION + EnumChatFormatting.WHITE + ")", 3, 3, getColor(colors, 1).getRGB(), fontOptions.isEnabled("Shadow"));
                ...
                getFont().drawString(EnumChatFormatting.BLACK + "Aint no way i cracked this shit", 4.2f, 41, Color.GRAY.getRGB(), fontOptions.isEnabled("Shadow"));
                getFont().drawString(EnumChatFormatting.RED + "Aint no way i cracked this shit", 3, 41, getColor(colors, 1).getRGB(), fontOptions.isEnabled("Shadow"));
                break;
            }
            case "Logo": {
                ...
                getFont().drawString("<- Shit pasted", icontestFont90.getStringWidth(FontUtils.LETTUCE) + 5, 20, Color.WHITE.getRGB(), fontOptions.isEnabled("Shadow"));
                break;
            }
        }
    }
    ...
    public AbstractFontRenderer getchatFont(boolean allowBold) {
        ...
        if (fontOptions.isEnabled("Bold") && allowBold) {
            switch (chatfont.getMode()) {
                default:
                    fontRenderer = mc.fontRendererObj;
                    break;
                case "Lettuce":
                    fontRenderer = lettuceBoldFont20;
                    break;
                case "Tahoma":
                    fontRenderer = tahomaBold20;
                    break;
                case "Neverlose":
                    fontRenderer = neverloseBold20;
                    break;
            };
        } else {
            switch (chatfont.getMode()) {
                default: fontRenderer = mc.fontRendererObj; break;
                case "Lettuce": fontRenderer = lettuceFont20; break;
                case "Tahoma": fontRenderer = tahoma20; break;
                case "Neverlose": fontRenderer = neverlose20; break;
            };
        }
        return fontRenderer;
    }
    ...
    public Color getColor(Color[] colors, int index) {
        ...
        switch (colorMode.getMode()) {
            default:
                ...
                break;
            case "Fade":
                color = ColorUtils.interpolateColorsBackAndForth(colorSpeed.getValue().intValue(), index, colors[0], colors[0].darker().darker(), false);
                break;
            case "Double Colors":
                color = ColorUtils.interpolateColorsBackAndForth(colorSpeed.getValue().intValue(), index, colors[0], colors[1], false);
                break;
            case "Rainbow":
                color = ColorUtils.rainbow(colorSpeed.getValue().intValue(), index, saturation.getValue().floatValue(), 1, 1);
                break;
        };
        return color;
    }
