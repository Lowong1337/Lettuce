...
public final class TargetStrafe extends Module {
    ...
    public static boolean canStrafe() {
        KillAura killAura = Lettuce.INSTANCE.getModuleManager().getModule(KillAura.class);
        if (!Lettuce.INSTANCE.getModuleManager().getModule(TargetStrafe.class).isEnabled() || !killAura.isEnabled()
                || !MoveUtils.isMoving() || (space.isEnabled() && !Keyboard.isKeyDown(Keyboard.KEY_SPACE))) {
            return false;
        }
        if (!(!Lettuce.INSTANCE.getModuleManager().getModule(Speed.class).isEnabled() || !Lettuce.INSTANCE.getModuleManager().getModule(Fly.class).isEnabled())) {
            return false;
        }
