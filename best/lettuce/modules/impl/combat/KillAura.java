...
public class KillAura extends Module {
    ...
    public final EventListener<EventMotion> onEvent = e -> {
        ...
        if (!targets.isEmpty() && targets.get(0) != null && mode.getMode().equals("Single")) {
            switch (sort.getMode()) {
                case "Range" -> targets.sort(Comparator.comparingDouble(mc.thePlayer::getDistanceToEntity));
                case "Health" -> targets.sort(Comparator.comparingDouble(EntityLivingBase::getHealth));
                case "Hurt time" -> targets.sort(Comparator.comparingInt(EntityLivingBase::getHurtTime));
                case "Armor" -> targets.sort(Comparator.comparingInt(EntityLivingBase::getTotalArmorValue));
            }
        }
        //Lettuce.INSTANCE.text(String.valueOf(targets.get(0).getName()));
        if (targets.size() > 0) {
            switch (mode.getMode()) {
                case "Single" -> targetmob = targets.get(0);
                case "Switch" -> {
    ...
    public void unblock() {
        if (blocking) {
            switch (abm.getMode()) {
                case "Grim", "Vanilla" -> PacketUtils.sendPacketNoEvent(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, EnumFacing.DOWN));
            }
        }
        blocking = false;
    }
