...
public class InventoryManager extends Module {
    ...
    private final EventListener<EventPacketSend> onSendPacket = event -> {
        ...
        if (packet instanceof C16PacketClientStatus clientStatus) {
            ...
            if (packetCloseWindow.getID() == this.mc.thePlayer.inventoryContainer.windowId) {
...
//            }
            ...
            if ((this.mc.currentScreen == null && this.mode.is("Spoof")) || (this.mc.currentScreen instanceof GuiInventory && this.mode.is("Open Inventory"))) {
                ...
                for (int slot = InventoryUtils.INCLUDE_ARMOR_BEGIN; slot < InventoryUtils.END; slot++) {
                    ...
                    if (stack != null) {
                        ...
                        } else if (stack.getItem() instanceof ItemArmor armor && InventoryUtils.isBestArmor(this.mc.thePlayer, stack)) {
                            final int pieceSlot = this.bestArmorPieces[armor.armorType];
                            if (pieceSlot == -1 || slot != pieceSlot) this.bestArmorPieces[armor.armorType] = slot;
    ...
    public void onEnable() {
        if (this.aura == null) {
            this.aura = Lettuce.INSTANCE.getModuleManager().getModule(KillAura.class);
        }
