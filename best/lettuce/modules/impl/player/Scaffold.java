...
public class Scaffold extends Module {
    ...
    private void doPlace(final EventMotion event) {
        ...
        if (this.mc.playerController.onPlayerRightClick(this.mc.thePlayer, this.mc.theWorld, heldItem, this.data.pos, this.data.face, hitVec)) {
            ...
            switch (this.swing.getMode()) {
                case "Client" -> this.mc.thePlayer.swingItem();
                case "Silent" -> this.mc.thePlayer.sendQueue.addToSendQueue(new C0APacketAnimation());
            }
        }
    }
    ...
    private static class BlockData {
        ...
        private Vec3 calculateBlockData() {
            ...
            switch (this.face.getAxis()) {
                case Z -> obj.hitVec = new Vec3(obj.hitVec.xCoord, obj.hitVec.yCoord, Math.round(obj.hitVec.zCoord));
                case X -> obj.hitVec = new Vec3(Math.round(obj.hitVec.xCoord), obj.hitVec.yCoord, obj.hitVec.zCoord);
            }
