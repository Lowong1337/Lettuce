...
public class LoginPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        for (ActionButton actionButton : actionButtons) {
            ...
            actionButton.setClickAction(() -> {
                switch (actionButton.getName()) {
                    case "Login" -> {
                        Lettuce.INSTANCE.getAltManager().getUtils().login(textFields.get(0), textFields.get(1));
                        resetTextFields();
                        Lettuce.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                    }
                    case "Random Cracked" -> {
                        Lettuce.INSTANCE.getAltManager().getUtils().loginWithString(generator.generate(8), "", false);
                        Lettuce.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                    }
                }
            });
        ...
        mc.getTextureManager().bindTexture(new ResourceLocation("Lettuce/mc.png"));
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (hoveringMicrosoft && button == 0) {
            ...
            Lettuce.INSTANCE.getAltManager().getUtils().microsoftLoginAsync(email, password);
            resetTextFields();
        }
    }
