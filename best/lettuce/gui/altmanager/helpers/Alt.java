...
public class Alt {
    ...
    private void login(boolean microsoft) {
        ...
        if ((!microsoft && this.password.isEmpty()) || a) {
            ...
            stage = 2;
            Lettuce.INSTANCE.getAltManager().currentSessionAlt = this;
            return;
        }
    ...
    public void loginAsync(boolean microsoft) {
        new Thread(() -> {
            ...
            new Thread(() -> {
                try {
                    Files.writeString(AltManagerUtils.altsFile.toPath(), new GsonBuilder().setPrettyPrinting().excludeFieldsWithoutExposeAnnotation().create().toJson(AltManagerUtils.getAlts().toArray(new Alt[0])));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }).start();
