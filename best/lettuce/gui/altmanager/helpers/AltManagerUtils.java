...
public class AltManagerUtils implements MinecraftInstance {
    ...
    public static File altsFile = new File(Lettuce.DIRECTORY, "alts.json");
        ...
        if (timerUtil.hasTimeElapsed(15000, true)) {
            new Thread(() -> {
                try {
                    ...
                    Files.writeString(altsFile.toPath(), new GsonBuilder().setPrettyPrinting().excludeFieldsWithoutExposeAnnotation().create().toJson(alts.toArray(new Alt[0])));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }).start();
    ...
    public void microsoftLoginAsync(Alt alt, String email, String password) {
        ...
        Multithreading.runAsync(() -> {
            ...
            if (auth != null) {
                ...
                writeAlts();
                Lettuce.INSTANCE.getAltManager().currentSessionAlt = finalAlt;
