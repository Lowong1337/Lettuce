...
public class ConfigRect implements Screen {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        float textAlpha = .5f;
        Color moduleTextColor = cfgname.equals(Lecture.INSTANCE.getConfigManager().getCurrentConfig()) ? ClickGUI.color.getColor() : ColorUtils.applyOpacity(textColor, textAlpha + (.4f * toggleAnimation.getOutput().floatValue()));
        ...
        lettuceFont18.drawString((cfgname.equals(Lecture.INSTANCE.getConfigManager().getCurrentConfig()) ? "§l" : "") + cfgname, x + 5, y + lettuceFont18.getMiddleOfBox(height), moduleTextColor);
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (hoveringModule) {
            switch (button) {
                case 0: {
                    toggleAnimation.setDirection(!cfgname.equals(Lecture.INSTANCE.getConfigManager().getCurrentConfig())  ? Direction.FORWARDS : Direction.BACKWARDS);
                    Lecture.INSTANCE.getConfigManager().loadConfig(cfgname, false);
                }
                break;
            }
        }
