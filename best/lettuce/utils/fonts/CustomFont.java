...
public class CustomFont implements AbstractFontRenderer {
    ...
    public float drawString(String text, double x, double y, int color, boolean shadow, boolean smooth) {
        if (text == null) {
            return 0;
        }
    ...
    public static void smooth(boolean smooth) {
        if (smooth) {
        ...
        for (int index = 0; index < text.length(); index++) {
            ...
            if (character == '§') {
                ...
                if (colorIndex < 16) {
                    ...
                    ColorUtils.color(colorCode[colorIndex], alpha);
                } else {
                    switch (colorIndex) {
                        case 17 -> {
                            if (hasBoldFont()) {
    ...
    public void drawLetter(double x, double y, FontData currentData, boolean strikethrough, boolean underline, char character) {
        ...
        drawQuad((float) x, (float) y, charData.width, charData.height, charData.storedX, charData.storedY, currentData.imageSize[0], currentData.imageSize[1]);
        GL11.glEnd();
    ...
    public int getStringWidth(String text) {
        return (int) getStringWidth(text, KERNING);
    }
        ...
        for (int index = 0; index < text.length(); index++) {
            ...
            if (character == '§') {
                ...
                switch (colorIndex) {
                    ...
                    case 20 -> currentData = this.regular.chars;
                    default -> currentData = regular.chars;
                }
                ++index;
    ...
    public List<String> getWrappedLines(String text, float x, float width) {
    ...
    public float[] drawNewLineText(String text, float x, float y, int color, float heightIncrement) {
        ...
        return new float[]{(float) getStringWidth(longest), newY - y};
    }
    ...
    private static class FontData {
        ...
        private final Integer[] imageSize = new Integer[]{512, 0};
    ...
    private void handleSprites(FontData fontData, Font currentFont, Graphics2D graphics2D, boolean drawString) {
        ...
        if (drawString) {
            BufferedImage image = new BufferedImage(fontData.imageSize[0], fontData.imageSize[1], BufferedImage.TYPE_INT_ARGB);
            Graphics2D graphics = (Graphics2D) image.getGraphics();

            graphics.setFont(currentFont);
            graphics.setColor(new Color(255, 255, 255, 0));
            graphics.fillRect(0, 0, fontData.imageSize[0], fontData.imageSize[1]);
            ...
            while (index < fontData.chars.length) {
                ...
                if (positionX + charData.width >= fontData.imageSize[0]) {
                    positionX = 0;
                ...
                fontData.imageSize[1] = positionY + fontMetrics.getAscent();
                ++index;
            }
