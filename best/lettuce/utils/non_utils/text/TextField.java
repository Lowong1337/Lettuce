package best.lettuce.utils.non_utils.text;
...
public class TextField extends Gui {
    ...
    @Setter @Getter
    private Color outline = Color.WHITE, fill = ColorUtils.tripleColor(32);
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        boolean flag = MouseUtils.isHovering(xPosition, yPosition, width, height, mouseX, mouseY);
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            ...
            if (textAlpha != 1) {
                textColorWithAlpha = ColorUtils.applyOpacity(focusedTextColor, textAlpha);
            }
            ...
            if (this.isDrawingBackground()) {
                if (outline != null) {
                    RoundedUtils.drawRound(this.xPosition - 1, this.yPosition - 1, this.width + 2f, this.height + 2f,
                            radius + 1, outline);
                }

                RoundedUtils.drawRound(this.xPosition, this.yPosition, this.width, this.height, radius, ColorUtils.applyOpacity(fill, alpha));
            } else {
                float rectHeight = 1;
                Gui.drawRect2(xPosition, yPosition + height - rectHeight, width, rectHeight,
                        ColorUtils.interpolateColor(focusedTextColor, unfocusedTextColor, textColor.getOutput().floatValue()));
            }

            textColor.setDirection(isFocused() ? Direction.BACKWARDS : Direction.FORWARDS);
            if (backgroundText != null) {
                Color backgroundTextColor = ColorUtils.applyOpacity(ColorUtils.applyOpacity(unfocusedTextColor, textAlpha), textColor.getOutput().floatValue());
                if (backgroundText.equals("Search")) {
                    FontUtils.iconFont16.drawString(FontUtils.SEARCH, xPos + 1.5f, this.yPosition + FontUtils.iconFont16.getMiddleOfBox(getHeight()), ColorUtils.applyOpacity(unfocusedTextColor, textAlpha));
                    xPos += 15;
                }
            ...
            if (canShowCursor) {
                ...
                Gui.drawRect2(k1 + 1, yPos - 2, .5f, font.getHeight() + 3,
                        ColorUtils.applyOpacity(textColorWithAlpha, cursorBlinkAnimation.getOutput().floatValue()).getRGB());
            }
