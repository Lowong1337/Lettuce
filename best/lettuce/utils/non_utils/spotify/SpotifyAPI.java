...
public class SpotifyAPI implements MinecraftInstance {
    ...
    public final static File CLIENT_ID_DIR = new File(Lettuce.DIRECTORY, "Spotify_ID.json");
    public final static File ACCESS_TOKEN = new File(Lettuce.DIRECTORY, "Spotify_TOKEN.json");
    ...
    private final SpotifyCallBack callback = code -> {
        Lettuce.text("Spotify: " + "Connecting to Spotify...");
    ...
    public void startConnection() {
        if (!authenticated) {
            try {
                ...
                Lettuce.INSTANCE.getExecutorService().submit(() -> {
                    try {
                        if (callbackServer != null) {
                            // Close the server if the module was disabled and re-enabled to prevent already bound exception
                            callbackServer.stop(0);
                        }
                        Lettuce.text("Spotify: " + "Please allow access to the application.");
    ...
    private void loadTokens() {
        try {
            if (ACCESS_TOKEN.exists()) {
                String storedTokens = Files.readString(ACCESS_TOKEN.toPath());
    ...
    public void saveTokens() {
        ...
        try {
            ...
            Path filePath = ACCESS_TOKEN.toPath();
            Files.writeString(filePath, jsonTokens, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void skipToPreviousTrack() {
        try {
            ...
            Lettuce.text("Spotify: " + e.getMessage());
        }
    }
