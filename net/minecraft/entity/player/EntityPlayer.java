...
public abstract class EntityPlayer extends EntityLivingBase {
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
        if (targetEntity.canAttackWithItem()) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag2) {
                        ...
                        if (i > 0) {
                            ...
                            if(!(aura.isEnabled() && aura.keepsprint.isEnabled())) {
                                this.motionX *= 0.6D;
                                this.motionZ *= 0.6D;
            ...
            try {
                EventAttack ea = new EventAttack((EntityLivingBase) targetEntity);
                Lettuce.INSTANCE.getEventManager().dispatch(ea);
            }
            catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void func_175139_a(EnumFacing p_175139_1_) {
        ...
        switch (p_175139_1_) {
            case SOUTH -> this.renderOffsetZ = -1.8F;
            case NORTH -> this.renderOffsetZ = 1.8F;
            case WEST -> this.renderOffsetX = 1.8F;
            case EAST -> this.renderOffsetX = -1.8F;
        }
    }
    ...
    public float getEyeHeight() {
        ...
        if (mc.thePlayer != null && tick != mc.thePlayer.ticksExisted || enableCameraYOffset) {
            ...
            if (enableCameraYOffset) {
                f = (float) (1.62F - (mc.thePlayer.lastTickPosY + (((mc.thePlayer.posY - mc.thePlayer.lastTickPosY) * mc.timer.renderPartialTicks)) - y));
            }
        }
