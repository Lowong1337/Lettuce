...
import best.lettuce.event.impl.player.EventJumpFix;
...
public abstract class EntityLivingBase extends Entity {
    ...
    private int getArmSwingAnimationEnd() {
        Animations animations = Lecture.INSTANCE.getModuleManager().getModule(Animations.class);
        int speed = (int) (6.0F * (animations.isEnabled() ? (animations.swingSpeed.getValue()).floatValue() : 1.0F));

        return isPotionActive(Potion.digSpeed) ? (speed - 1 + getActivePotionEffect(Potion.digSpeed).getAmplifier()) : (isPotionActive(Potion.digSlowdown) ? (speed + (1 + getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2) : speed);
    }
    ...
    protected void jump() {
        ...
        if (this.isSprinting()) {
            EventJumpFix jump = new EventJumpFix(this.rotationYaw);

            if(this instanceof EntityPlayerSP){
                Lecture.INSTANCE.getEventManager().dispatch(jump);
            }
