...
public class GameSettings {
    ...
    public boolean ofSmoothWorld = OFConfig.isSingleProcessor();
    public boolean ofLazyChunkLoading = OFConfig.isSingleProcessor();
    ...
    public GameSettings(Minecraft mcIn, File optionsFileIn) {
        ...
        this.loadOptions();
        OFConfig.initGameSettings(this);
    }
    ...
    public void setOptionValue(GameSettings.Options settingsOption, int value) {
        ...
        if (settingsOption == GameSettings.Options.GUI_SCALE) {
            ...
            DisplayMode displaymode = OFConfig.getLargestDisplayMode();
        ...
        if (settingsOption == GameSettings.Options.ANAGLYPH) {
            if (!this.anaglyph && OFConfig.isShaders()) {
                OFConfig.showGuiMessage(Lang.get("of.message.an.shaders1"), Lang.get("of.message.an.shaders2"));
                return;
            }
    ...
    public float getOptionFloatValue(GameSettings.Options settingOption) {
        ...
        return f != Float.MAX_VALUE ? f : (settingOption == GameSettings.Options.FOV ? this.fovSetting : (settingOption == GameSettings.Options.GAMMA ?
                this.gammaSetting : (settingOption == GameSettings.Options.SATURATION ? this.saturation : (settingOption == GameSettings.Options.SENSITIVITY ?
                this.mouseSensitivity : (settingOption == GameSettings.Options.CHAT_OPACITY ? this.chatOpacity :
                (settingOption == GameSettings.Options.CHAT_HEIGHT_FOCUSED ? this.chatHeightFocused :
                        (settingOption == GameSettings.Options.CHAT_HEIGHT_UNFOCUSED ? this.chatHeightUnfocused :
                                (settingOption == GameSettings.Options.CHAT_SCALE ? this.chatScale : (settingOption == GameSettings.Options.CHAT_WIDTH ?
                                        this.chatWidth : (settingOption == GameSettings.Options.FRAMERATE_LIMIT ? (float) this.limitFramerate :
                                        (settingOption == GameSettings.Options.MIPMAP_LEVELS ? (float) this.mipmapLevels :
                                                (settingOption == GameSettings.Options.RENDER_DISTANCE ? (float) this.renderDistanceChunks :
                                                        0.0f))))))))))));
    }
    ...
    public boolean getOptionOrdinalValue(GameSettings.Options settingOption) {
        return switch (settingOption) {
            ...
            case REALMS_NOTIFICATIONS -> this.realmsNotifications;
            default -> false;
        };
    }
    ...
    private void setOptionFloatValueOF(GameSettings.Options p_setOptionFloatValueOF_1_, float p_setOptionFloatValueOF_2_) {
        ...
        if (p_setOptionFloatValueOF_1_ == GameSettings.Options.AA_LEVEL) {
            ...
            if (i > 0 && OFConfig.isShaders()) {
                OFConfig.showGuiMessage(Lang.get("of.message.aa.shaders1"), Lang.get("of.message.aa.shaders2"));
                return;
            }
    ...
    private float getOptionFloatValueOF(GameSettings.Options p_getOptionFloatValueOF_1_) {
        if (p_getOptionFloatValueOF_1_ == GameSettings.Options.CLOUD_HEIGHT) {
            ...
            if (this.ofFullscreenMode.equals("Default")) {
                ...
                List list = Arrays.asList(OFConfig.getDisplayModeNames());
    ...
    private void setOptionValueOF(GameSettings.Options p_setOptionValueOF_1_, int p_setOptionValueOF_2_) {
        if (p_setOptionValueOF_1_ == GameSettings.Options.FOG_FANCY) {
            switch (this.ofFogType) {
                case 1 -> {
                    this.ofFogType = 2;
                    if (!OFConfig.isFancyFogAvailable()) {
                        this.ofFogType = 3;
                    }
                }
        ...
        if (p_setOptionValueOF_1_ == GameSettings.Options.SMOOTH_WORLD) {
            ...
            OFConfig.updateThreadPriorities();
        }
        ...
        if (p_setOptionValueOF_1_ == GameSettings.Options.CUSTOM_FONTS) {
            ...
            this.mc.fontRendererObj.onResourceManagerReload(OFConfig.getResourceManager());
            this.mc.standardGalacticFontRenderer.onResourceManagerReload(OFConfig.getResourceManager());
        }
        ...
        //OFConfig.updateFramebufferSize();
    ...
    private String getKeyBindingOF(GameSettings.Options p_getKeyBindingOF_1_) {
        ...
        if (p_getKeyBindingOF_1_ == GameSettings.Options.RENDER_DISTANCE) {
            ...
            if (this.ofAaLevel != OFConfig.getAntialiasingLevel()) {
    ...
    public void loadOfOptions() {
        try {
            ...
            while ((s = bufferedreader.readLine()) != null) {
                try {
                    ...
                    if (astring[0].equals("ofRenderDistanceChunks") && astring.length >= 2) {
                        this.renderDistanceChunks = Integer.parseInt(astring[1]);
                        this.renderDistanceChunks = OFConfig.limit(this.renderDistanceChunks, 2, 1024);
                    }

                    if (astring[0].equals("ofFogType") && astring.length >= 2) {
                        this.ofFogType = Integer.parseInt(astring[1]);
                        this.ofFogType = OFConfig.limit(this.ofFogType, 1, 3);
                    }
                    ...
                    if (astring[0].equals("ofMipmapType") && astring.length >= 2) {
                        ...
                        this.ofMipmapType = OFConfig.limit(this.ofMipmapType, 0, 3);
                    }
                    ...
                    if (astring[0].equals("ofVignette") && astring.length >= 2) {
                        ...
                        this.ofVignette = OFConfig.limit(this.ofVignette, 0, 2);
                    }

                    if (astring[0].equals("ofChunkUpdates") && astring.length >= 2) {
                        this.ofChunkUpdates = Integer.parseInt(astring[1]);
                        this.ofChunkUpdates = OFConfig.limit(this.ofChunkUpdates, 1, 5);
                    }
                    ...
                    if (astring[0].equals("ofTime") && astring.length >= 2) {
                        ...
                        this.ofTime = OFConfig.limit(this.ofTime, 0, 2);
                    }
                    ...
                    if (astring[0].equals("ofScreenshotSize") && astring.length >= 2) {
                        ...
                        this.ofScreenshotSize = OFConfig.limit(this.ofScreenshotSize, 1, 4);
                    }
                    ...
                    if (astring[0].equals("ofTranslucentBlocks") && astring.length >= 2) {
                        ...
                        this.ofTranslucentBlocks = OFConfig.limit(this.ofTranslucentBlocks, 0, 2);
                    }
                    ...
                    OFConfig.dbg("Skipping bad option: " + s);
                    exception.printStackTrace();
                }
            }
            ...
            OFConfig.warn("Failed to load options");
    ...
    public void saveOfOptions() {
        try {
            ...
            OFConfig.warn("Failed to save options");
            exception.printStackTrace();
        }
    }
    ...
    private void updateRenderClouds() {
        switch (this.ofClouds) {
            case 1 -> this.clouds = 1;
            case 2 -> this.clouds = 2;
            case 3 -> this.clouds = 0;
            default -> {
                if (this.fancyGraphics) {
                    this.clouds = 2;
                } else {
    ...
    public void resetSettings() {
        ...
        OFConfig.updateAvailableProcessors();
        this.ofSmoothWorld = OFConfig.isSingleProcessor();
    ...
    private void updateWaterOpacity() {
        if (OFConfig.isIntegratedServerRunning()) {
            OFConfig.waterOpacityChanged = true;
        }
