...
import best.lettuce.event.impl.render.EventPreRender;
import best.lettuce.event.impl.render.EventRender2D;
import best.lettuce.modules.impl.misc.StreamerMode;
import best.lettuce.modules.impl.render.Camera;
import best.lettuce.modules.impl.render.HUD;
import best.lettuce.modules.impl.render.Scoreboard;
import best.lettuce.modules.impl.render.VisualEffects;
...
public class GuiIngame extends Gui {
    ...
    public AbstractFontRenderer getFontRenderer() {
        Scoreboard scoreboard = Lecture.INSTANCE.getModuleManager().getModule(Scoreboard.class);
        AbstractFontRenderer fontRenderer;
        switch (scoreboard.font.getMode()) {
            default: fontRenderer = mc.fontRendererObj; break;
            case "Lettuce": fontRenderer = MinecraftInstance.lettuceFont20; break;
            case "Tahoma": fontRenderer = MinecraftInstance.tahoma20; break;
            case "Neverlose": fontRenderer = MinecraftInstance.neverlose20; break;
        };
        return fontRenderer;
    }
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        Lecture.INSTANCE.getEventManager().dispatch(new EventPreRender());

        VisualEffects visualEffects = Lecture.INSTANCE.getModuleManager().getModule(VisualEffects.class);
        visualEffects.blurScreen();

        Lecture.INSTANCE.getEventManager().dispatch(new EventRender2D(scaledresolution.getWidth(), scaledresolution.getHeight(), partialTicks));
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            for (int j = 0; j < 9; ++j) {
                int k = sr.getWidth() / 2 - 90 + j * 20 + 2;
                int l = sr.getHeight() - 16 - 3;
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        if (scoreboard.isEnabled()) {
            renderscore(objective, scaledRes);
        }
    }

    private void renderscore(ScoreObjective objective, ScaledResolution scaledRes) {
        SimpleDateFormat format = new SimpleDateFormat("MM/dd/yy");
        StreamerMode streamerMode = Lecture.INSTANCE.getModuleManager().getModule(StreamerMode.class);
        Scoreboard sb = Lecture.INSTANCE.getModuleManager().getModule(Scoreboard.class);
        ...
        int i1 = collection.size() * 8;
        int j1 = scaledRes.getHeight() / 2 + i1 / 3;
        int k1 = 3;
        int l1 = scaledRes.getWidth() - i - k1;
        int j = 0;

        for (Score score1 : collection) {
            ++j;
            ScorePlayerTeam scoreplayerteam1 = scoreboard.getPlayersTeam(score1.getPlayerName());
            String[] blacklisted = new String[]{".com", ".net", ".com", ".es", ".us", ".wtf", ".vn", ".de", ".fun", ".cn", ".eu", ".club", ".games", "mc.hypixel.net"};
            ...
            s1 = s1.replace(mc.thePlayer.getName(), streamerMode.name.getString() + EnumChatFormatting.AQUA + " - Tsukasa.tokyo");
            s2 = s2.replace(mc.thePlayer.getName(), streamerMode.name.getString() + EnumChatFormatting.AQUA + " - Tsukasa.tokyo");
            if (streamerMode.isEnabled() && streamerMode.hideothers.isEnabled()) {
                for (EntityPlayer entityPlayer : mc.theWorld.playerEntities) {
                    s1 = s1.replace(entityPlayer.getName(), "Player " + mc.theWorld.playerEntities.indexOf(entityPlayer) + 1);
                    s2 = s2.replace(entityPlayer.getName(), "Player " + mc.theWorld.playerEntities.indexOf(entityPlayer) + 1);
                }
            }
            int k = j1 - j * 8;
            int l = scaledRes.getWidth() - k1 + 2;
            switch (sb.font.getMode()) {
                case "Lettuce": {
                    drawRect(l1 - 2, k + sb.yOffset.getValue().floatValue(), l, k + getFontRenderer().getHeight() + sb.yOffset.getValue().floatValue() - 1f, new Color(0, 0, 0, 127).getRGB());
                    //this.getFontRenderer().drawString(s1, l1, k + sb.yOffset.getValue().floatValue(), 553648127);
                    Lecture.lettuceFont18.drawString(s1, l1, k + sb.yOffset.getValue().floatValue() + 0.5f, Color.WHITE);
                    //Lettuce.text(s1);
                    if (sb.redNumbers.isEnabled()) {
                        Lecture.lettuceFont16.drawString(s2, l - Lecture.lettuceFont16.getStringWidth(s2), k + sb.yOffset.getValue().floatValue(), s1.contains("lettuce") ? HUD.color : Color.WHITE);
                    }

                    if (j == collection.size()) {
                        String s3 = objective.getDisplayName();
                        drawRect(l1 - 2, k - getFontRenderer().getHeight() - 1 + sb.yOffset.getValue().floatValue(), l, k - 1 + sb.yOffset.getValue().floatValue(), new Color(0, 0, 0, 127).getRGB());
                        drawRect(l1 - 2, k - 1 + sb.yOffset.getValue().floatValue(), l, k + sb.yOffset.getValue().floatValue(), new Color(0, 0, 0, 127).getRGB());
                        Lecture.lettuceFont18.drawString(s3, l1 + i / 2 - (float) this.getFontRenderer().getStringWidth(s3) / 2, k - getFontRenderer().getHeight() + sb.yOffset.getValue().floatValue(), Color.WHITE);
                    }
                    break;
                }
                case "Minecraft": {
                    drawRect(l1 - 2, k + sb.yOffset.getValue().floatValue(), l, k + getFontRenderer().getHeight() + sb.yOffset.getValue().floatValue() - 1, 1342177280);
                    this.getFontRenderer().drawString(s1, l1, k + sb.yOffset.getValue().floatValue(), 553648127);
                    if (sb.redNumbers.isEnabled()) {
                        this.getFontRenderer().drawString(s2, l - this.getFontRenderer().getStringWidth(s2), k + sb.yOffset.getValue().floatValue(), s1.contains("lettuce") ? HUD.color : Color.WHITE);
                    }

                    if (j == collection.size()) {
                        String s3 = objective.getDisplayName();
                        drawRect(l1 - 2, k - 8 - 1 + sb.yOffset.getValue().floatValue(), l, k - 1 + sb.yOffset.getValue().floatValue(), 1610612736);
                        drawRect(l1 - 2, k - 1 + sb.yOffset.getValue().floatValue(), l, k + sb.yOffset.getValue().floatValue(), 1342177280);
                        this.getFontRenderer().drawString(s3, l1 + i / 2 - (float) this.getFontRenderer().getStringWidth(s3) / 2, k - 8 + sb.yOffset.getValue().floatValue(), 553648127);
                    }
                    break;
                }
                case "Tahoma": {
                    drawRect(l1 - 2, k + sb.yOffset.getValue().floatValue(), l, k + getFontRenderer().getHeight() + sb.yOffset.getValue().floatValue() - 1, new Color(0, 0, 0, 127).getRGB());
                    //this.getFontRenderer().drawString(s1, l1, k + sb.yOffset.getValue().floatValue(), 553648127);
                    Lecture.tahoma18.drawString(s1, l1, k + sb.yOffset.getValue().floatValue() + 1f, Color.WHITE);
                    if (sb.redNumbers.isEnabled()) {
                        Lecture.tahoma18.drawString(s2, l - Lecture.tahoma18.getStringWidth(s2), k + sb.yOffset.getValue().floatValue() + 1f, s1.contains("lettuce") ? HUD.color : Color.WHITE);
                    }
                    ...
                    if (j == collection.size()) {
                        ...
                        drawRect(l1 - 2, k - getFontRenderer().getHeight() - 1 + sb.yOffset.getValue().floatValue(), l, k - 1 + sb.yOffset.getValue().floatValue(), new Color(0, 0, 0, 127).getRGB());
                        drawRect(l1 - 2, k - 1 + sb.yOffset.getValue().floatValue(), l, k + sb.yOffset.getValue().floatValue(), new Color(0, 0, 0, 127).getRGB());
                        Lecture.tahoma18.drawString(s3, l1 + i / 2 - (float) this.getFontRenderer().getStringWidth(s3) / 2, k - getFontRenderer().getHeight() + sb.yOffset.getValue().floatValue() + 1f, Color.WHITE);
                    }
                    break;
                }
                case "Neverlose": {
                    drawRect(l1 - 2, k + sb.yOffset.getValue().floatValue(), l, k + getFontRenderer().getHeight() + sb.yOffset.getValue().floatValue(), new Color(0, 0, 0, 127).getRGB());
                    //this.getFontRenderer().drawString(s1, l1, k + sb.yOffset.getValue().floatValue(), 553648127);
                    Lecture.neverlose20.drawString(s1, l1, k + sb.yOffset.getValue().floatValue() + 0.5f, Color.WHITE);
                    if (sb.redNumbers.isEnabled()) {
                        Lecture.neverlose20.drawString(s2, l - Lecture.neverlose20.getStringWidth(s2), k + sb.yOffset.getValue().floatValue() + 1, s1.contains("lettuce") ? HUD.color : Color.WHITE);
                    }
                    ...
                    if (j == collection.size()) {
                        ...
                        drawRect(l1 - 2, k - getFontRenderer().getHeight() - 1 + sb.yOffset.getValue().floatValue(), l, k - 1 + sb.yOffset.getValue().floatValue(), new Color(0, 0, 0, 127).getRGB());
                        drawRect(l1 - 2, k - 1 + sb.yOffset.getValue().floatValue(), l, k + sb.yOffset.getValue().floatValue(), new Color(0, 0, 0, 127).getRGB());
                        Lecture.neverlose20.drawString(s3, l1 + i / 2 - (float) this.getFontRenderer().getStringWidth(s3) / 2, k - getFontRenderer().getHeight() + sb.yOffset.getValue().floatValue() + 1, Color.WHITE);
                    }
                    break;
                }
            }
    ...
    private void renderPlayerStats(ScaledResolution scaledRes) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            int j1 = scaledRes.getWidth() / 2 + 91;
            int k1 = scaledRes.getHeight() - 39;
    ...
    private void renderPumpkinOverlay(ScaledResolution scaledRes) {
        Camera camera = Lecture.INSTANCE.getModuleManager().getModule(Camera.class);
        if (camera.isEnabled() || !camera.options.isEnabled("Remove Pumpkin Overlay")) {
            pumpkin(scaledRes);
        }
    }

    private void pumpkin(ScaledResolution scaledRes) {
        GlStateManager.disableDepth();
        ...
        worldrenderer.pos(0.0D, scaledRes.getHeight(), -90.0D).tex(0.0D, 1.0D).endVertex();
        worldrenderer.pos(scaledRes.getWidth(), scaledRes.getHeight(), -90.0D).tex(1.0D, 1.0D).endVertex();
        worldrenderer.pos(scaledRes.getWidth(), 0.0D, -90.0D).tex(1.0D, 0.0D).endVertex();
    ...
    private void renderPortal(float timeInPortal, ScaledResolution scaledRes) {
        ...
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
        worldrenderer.pos(0.0D, scaledRes.getHeight(), -90.0D).tex(f, f3).endVertex();
        worldrenderer.pos(scaledRes.getWidth(), scaledRes.getHeight(), -90.0D).tex(f2, f3).endVertex();
        worldrenderer.pos(scaledRes.getWidth(), 0.0D, -90.0D).tex(f2, f1).endVertex();
    ...
    public void renderScoreboardBlur(ScaledResolution scaledRes) {
        Scoreboard modScoreboard = Lecture.INSTANCE.getModuleManager().getModule(Scoreboard.class);
        net.minecraft.scoreboard.Scoreboard scoreboardOBJ = this.mc.theWorld.getScoreboard();
        ScoreObjective scoreobjective = null;
        ...
        if (objective != null && modScoreboard.isEnabled()) {
            ...
            int i1 = collection.size() * this.getFontRenderer().getHeight();
            int j1 = scaledRes.getHeight() / 2 + i1 / 3 + modScoreboard.yOffset.getValue().intValue();
            int k1 = 3;
            float l1 = scaledRes.getWidth() - i - k1;
            int j = 0;

            for (Score ignored : collection) {
                ++j;
                int k = j1 - j * this.getFontRenderer().getHeight();
                int l = scaledRes.getWidth() - k1 + 2;
                drawRect(l1 - 2, k, l, k + this.getFontRenderer().getHeight(), Color.BLACK.getRGB());

                if (j == collection.size()) {
                    //drawRect(l1 - 2, k - this.getFontRenderer().getHeight(), l, k, Color.BLACK.getRGB());
                    drawRect(l1 - 2, k - 2f, l, k, Color.BLACK.getRGB());
                }
            }
        }
    }
