...
import best.lettuce.modules.impl.player.ChestStealer;
...
public abstract class GuiContainer extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ChestStealer stealer = Lecture.INSTANCE.getModuleManager().getModule(ChestStealer.class);
        if (stealer.isEnabled()) {
            if (mc.thePlayer.openContainer instanceof ContainerChest) {
                if (stealer.silent.isEnabled()) {
                    Lecture.lettuceFont22.drawCenteredString("Stealing chest, press E or ESC to cancel.", this.width / 2f, this.height / 2f + 15, Color.WHITE.getRGB());
                    //NotificationManager.post(NotificationType.INFO, "Chest Stealer", "Stealing chest, press E or ESC to cancel", 1);
                } else {
                    rendercontainer(mouseX, mouseY, partialTicks);
                }
            } else {
                rendercontainer(mouseX, mouseY, partialTicks);
            }
    ...
    public void rendercontainer(int mouseX, int mouseY, float partialTicks) {
        ...
        if (slotIn == this.clickedSlot && this.draggedStack != null && this.isRightMouseClick && itemstack != null) {
            ...
            if (Container.canAddItemToSlot(slotIn, itemstack1, true) && this.inventorySlots.canDragIntoSlot(slotIn)) {
                ...
                if (itemstack.stackSize > itemstack.getMaxStackSize()) {
                    s = EnumChatFormatting.YELLOW + String.valueOf(itemstack.getMaxStackSize());
                    itemstack.stackSize = itemstack.getMaxStackSize();
                }

                if (itemstack.stackSize > slotIn.getItemStackLimit(itemstack)) {
                    s = EnumChatFormatting.YELLOW + String.valueOf(slotIn.getItemStackLimit(itemstack));
