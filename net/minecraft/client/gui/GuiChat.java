...
import best.lettuce.modules.impl.render.Spotify;
...
public class GuiChat extends GuiScreen {
    ...
    private final List<MenuButton> buttons = Arrays.asList(
            new MenuButton("Reset Positions"),
            new MenuButton("Open ClickGUI")
    );
    ...
    public void initGui() {
        ...
        if (
            spotifyMod == null ||
                arraylistMod == null) {
            spotifyMod = (Spotify) Lecture.INSTANCE.getModuleManager().get(Spotify.class);
            arraylistMod = (HUD) Lecture.INSTANCE.getModuleManager().get(HUD.class);
        }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        Gui.drawRect2(2, this.height - 14, this.width - 4, 12, Integer.MIN_VALUE);

        HUD HUD = Lecture.INSTANCE.getModuleManager().getModule(HUD.class);
        inputField.font = HUD.getFont(false);

        inputField.yPosition = this.height - 12;
        ...
        for (MenuButton button : buttons) {
            ...
            button.y = ((height / 2f - buttonHeight / 2f) - 10) + count;
            ...
            button.clickAction = () -> {
                switch (button.text) {
                    ...
                    case "Open ClickGUI" : {
                        mc.displayGuiScreen(null);
                        Lecture.INSTANCE.getModuleManager().getModule(ClickGUI.class).toggle();
                        break;
                    }
                }
            };
        ...
        if (spotifyMod.isEnabled()) {
            ...
            for (String button : spotifyMod.buttons) {
                CustomFont iconFont = FontUtils.FontType.ICONTEST.size(20);
                boolean hover = MouseUtils.isHovering(spotifyDrag.getX() + spotifyMod.albumCoverSize + 6 + spacing, spotifyDrag.getY() + spotifyMod.height - (19),
            ...
            spotifyMod.hoveringPause = MouseUtils.isHovering(spotifyDrag.getX(), spotifyDrag.getY(), spotifyMod.albumCoverSize, spotifyMod.height, mouseX, mouseY);
        }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (spotifyMod.isEnabled()) {
            ...
            for (String button : spotifyMod.buttons) {
                Dragging spotifyDrag = DragManager.draggables.get("spotify");
                CustomFont iconFont = FontUtils.FontType.ICON.size(20);
                if (MouseUtils.isHovering(spotifyDrag.getX() + spotifyMod.albumCoverSize + 6 + spacing, spotifyDrag.getY() + spotifyMod.height - (19),
                        (float) iconFont.getStringWidth(button), iconFont.getHeight(), mouseX, mouseY)) {
                    if (mouseButton == 0) {
                        Multithreading.runAsync(() -> {
                            switch (button) {
                                case FontUtils.SKIP_LEFT: spotifyMod.api.skipToPreviousTrack(); break;
                                case FontUtils.SKIP_RIGHT: spotifyMod.api.skipTrack(); break;
                                case FontUtils.SHUFFLE: spotifyMod.api.toggleShuffleState(); break;
                            }
                        });
                        return;
                    }
