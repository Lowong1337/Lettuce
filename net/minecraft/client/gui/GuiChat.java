...
public class GuiChat extends GuiScreen {
    ...
    public void initGui() {
        ...
        if (
            spotifyMod == null ||
                arraylistMod == null) {
            spotifyMod = (Spotify) Lettuce.INSTANCE.getModuleManager().get(Spotify.class);
            arraylistMod = (HUD) Lettuce.INSTANCE.getModuleManager().get(HUD.class);
        }
        ...
        for (MenuButton button : buttons) {
            ...
            button.clickAction = () -> {
                switch (button.text) {
                    ...
                    case "Open ClickGUI" -> {
                        mc.displayGuiScreen(null);
                        Lettuce.INSTANCE.getModuleManager().getModule(ClickGUI.class).toggle();
                    }
                }
            };
                ...
                if (MouseUtils.isHovering(spotifyDrag.getX() + spotifyMod.albumCoverSize + 6 + spacing, spotifyDrag.getY() + spotifyMod.height - (19),
                    ...
                    if (mouseButton == 0) {
                        Multithreading.runAsync(() -> {
                            switch (button) {
                                case FontUtils.SKIP_LEFT -> spotifyMod.api.skipToPreviousTrack();
                                case FontUtils.SKIP_RIGHT -> spotifyMod.api.skipTrack();
                                case FontUtils.SHUFFLE -> spotifyMod.api.toggleShuffleState();
                            }
                        });
                        return;
                    }
