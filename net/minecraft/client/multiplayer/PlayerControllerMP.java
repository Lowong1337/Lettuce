...
import best.lettuce.event.impl.player.EventCurrentItemHeld;
import best.lettuce.event.impl.player.EventPlaceBlock;
...
public class PlayerControllerMP {
    ...
    public void syncCurrentPlayItem() {
        final EventCurrentItemHeld event = new EventCurrentItemHeld(this.mc.thePlayer.inventory.currentItem);
        Lettuce.INSTANCE.getEventManager().dispatch(event);
    ...
    public boolean onPlayerRightClick(EntityPlayerSP player, WorldClient worldIn, ItemStack heldStack, BlockPos hitPos, EnumFacing side, Vec3 hitVec) {
        ...
        if (!this.mc.theWorld.getWorldBorder().contains(hitPos)) {
            ...
            final EventPlaceBlock event = new EventPlaceBlock(hitPos, side, hitVec);
            Lettuce.INSTANCE.getEventManager().dispatch(event);
    ...
    public void windowClick(int windowId, int slotId, int mouseButtonClicked, int mode, EntityPlayer playerIn) {
        EventWindowClick event = new EventWindowClick(windowId, slotId, mouseButtonClicked, mode);
        Lettuce.INSTANCE.getEventManager().dispatch(event);
        ItemStack itemstack = playerIn.openContainer.slotClick(slotId, mouseButtonClicked, mode, playerIn);
        this.netClientHandler.addToSendQueue(new C0EPacketClickWindow(windowId, slotId, mouseButtonClicked, mode, itemstack, playerIn.openContainer.getNextTransactionID(playerIn.inventory)));
    }
    ...
    public boolean placeBlock(int itemSlot, int blockX, int blockY, int blockZ, int side, double hitX, double hitY, double hitZ) {
        Minecraft mc = Minecraft.getMinecraft();
        if (mc.thePlayer == null)
            return false;

        ItemStack itemStack = mc.thePlayer.inventory.getStackInSlot(itemSlot);
        BlockPos blockPos = new BlockPos(blockX, blockY, blockZ);
        EnumFacing facing = getFacingFromHitVec(blockPos, side, hitX, hitY, hitZ);

        if (itemStack != null) {
            Item item = itemStack.getItem();
            return item.onItemUse(itemStack, mc.thePlayer, mc.theWorld, blockPos, facing, (float) hitX, (float) hitY, (float) hitZ);
        }

        return false;
    }

    public EnumFacing getFacingFromHitVec(BlockPos blockPos, int side, double hitX, double hitY, double hitZ) {
        Vec3 hitVec = new Vec3(hitX, hitY, hitZ);
        Vec3 blockVec = new Vec3(blockPos.getX(), blockPos.getY(), blockPos.getZ());
        Vec3 facingVec = hitVec.subtract(blockVec);

        double xDiff = Math.abs(facingVec.xCoord);
        double yDiff = Math.abs(facingVec.yCoord);
        double zDiff = Math.abs(facingVec.zCoord);

        EnumFacing facing;

        if (xDiff > yDiff && xDiff > zDiff) {
            facing = facingVec.xCoord > 0 ? EnumFacing.WEST : EnumFacing.EAST;
        } else if (yDiff > zDiff) {
            facing = facingVec.yCoord > 0 ? EnumFacing.DOWN : EnumFacing.UP;
        } else {
            facing = facingVec.zCoord > 0 ? EnumFacing.NORTH : EnumFacing.SOUTH;
        }
