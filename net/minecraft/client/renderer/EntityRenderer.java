...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            if (flag) {
                if (!OFConfig.zoomMode) {
                    OFConfig.zoomMode = true;
                    OFConfig.zoomSmoothCamera = this.mc.gameSettings.smoothCamera;
                    this.mc.gameSettings.smoothCamera = true;
                    this.mc.renderGlobal.displayListEntitiesDirty = true;
                }

                f /= 4.0F;
            } else if (OFConfig.zoomMode) {
                OFConfig.zoomMode = false;
                this.mc.gameSettings.smoothCamera = OFConfig.zoomSmoothCamera;
    ...
    private void hurtCameraEffect(float partialTicks) {
        ...
        if (camera.isEnabled()) {
            if (camera.hurtCamera.is("Vanilla")) drawHurtCamera(partialTicks);
        } else {
            this.drawHurtCamera(partialTicks);
        }
    }

    private void drawHurtCamera(float partialTicks) {
    ...
    public void setupCameraTransform(float partialTicks, int pass) {
        ...
        if (OFConfig.isFogFancy()) {
            this.farPlaneDistance *= 0.95F;
        }

        if (OFConfig.isFogFast()) {
                ...
                if (flag1 && this.mc.gameSettings.thirdPersonView == 0 && !flag && !this.mc.gameSettings.hideGUI && !this.mc.playerController.isSpectator()) {
                    ...
                    if (OFConfig.isShaders()) {
                        if (mc.thePlayer.getHeldItem() != null) {
                            KillAura aura = Lecture.INSTANCE.getModuleManager().getModule(KillAura.class);
                            if (aura.autoblock.isEnabled() && aura.abm.is("Fake") && KillAura.isattacking && mc.thePlayer.inventory.getCurrentItem().getItem() != null && mc.thePlayer.inventory.getCurrentItem().getItem() instanceof ItemSword) {
                                this.itemRenderer.renderblocking(p_renderHand_1_);
                            } else {
    ...
    private void updateLightmap(float partialTicks) {
        if (this.lightmapUpdateNeeded) {
            ...
            if (world != null) {
                if (OFConfig.isCustomColors() && CustomColors.updateLightmap(world, this.torchFlickerX, this.lightmapColors, this.mc.thePlayer.isPotionActive(Potion.nightVision), partialTicks)) {
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        OFConfig.renderPartialTicks = partialTicks;
        this.frameInit();
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    crashreportcategory.addCrashSectionCallable("Screen size", () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", scaledresolution.getWidth(), scaledresolution.getHeight(), EntityRenderer.this.mc.displayWidth, EntityRenderer.this.mc.displayHeight, scaledresolution.getScaleFactor()));
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        boolean flag = OFConfig.isShaders();
        ...
        clippinghelper.disabled = OFConfig.isShaders() && !Shaders.isFrustumCulling();
        ...
        if ((OFConfig.isSkyEnabled() || OFConfig.isSunMoonEnabled() || OFConfig.isStarsEnabled()) && !Shaders.isShadowPass) {
        ...
        Lecture.INSTANCE.getEventManager().dispatch(new EventRender3D(partialTicks));
    ...
    private void renderCloudsCheck(RenderGlobal renderGlobalIn, float partialTicks, int pass) {
        if (this.mc.gameSettings.renderDistanceChunks >= 4 && !OFConfig.isCloudsOff() && Shaders.shouldRenderClouds(this.mc.gameSettings)) {
    ...
    private void addRainParticles() {
        ...
        if (!OFConfig.isRainFancy()) {
            f /= 2.0F;
        }

        if (f != 0.0F && OFConfig.isRainSplash()) {
    ...
    private void updateFogColor(float partialTicks) {
        ...
        if (this.cloudFog) {
            ...
            f12 = OFConfig.limit(f12, 0.0F, 0.6F);
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(Potion.blindness)) {
            if (Lecture.INSTANCE.getModuleManager().getModule(Camera.class).isEnabled() && Lecture.INSTANCE.getModuleManager().getModule(Camera.class).options.isEnabled("Remove Blindness Effect"))
                return;
    ...
    private void setupFog(int startCoords, float partialTicks) {
        ...
        if (f >= 0.0F) {
            ...
            if (GLContext.getCapabilities().GL_NV_fog_distance && OFConfig.isFogFancy()) {
            ...
            GlStateManager.setFog(2048);
            float f1 = OFConfig.isClearWater() ? 0.02F : 0.1F;

            if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(Potion.waterBreathing)) {
                GlStateManager.setFogDensity(0.01F);
            } else {
                float f2 = 0.1F - (float) EnchantmentHelper.getRespiration(entity) * 0.03F;
                GlStateManager.setFogDensity(OFConfig.limit(f2, 0.0F, f1));
            }
        } else if (block.getMaterial() == Material.lava) {
            GlStateManager.setFog(2048);
            ...
            if (startCoords == -1) {
                ...
                GlStateManager.setFogStart(f3 * OFConfig.getFogStart());
                GlStateManager.setFogEnd(f3);
            }
    ...
    private void waitForServerThread() {
        ...
        if (OFConfig.isSmoothWorld() && OFConfig.isSingleProcessor()) {
                ...
                if (integratedserver != null) {
                    ...
                    if (!flag && !(this.mc.currentScreen instanceof GuiDownloadTerrain)) {
                        if (this.serverWaitTime > 0) {
                            ...
                            OFConfig.sleep(this.serverWaitTime);
    ...
    private void frameInit() {
        ...
        if (!this.initialized) {
            ...
            if (OFConfig.getBitsOs() == 64 && OFConfig.getBitsJre() == 32) {
                OFConfig.setNotify64BitJava(true);
            }

            this.initialized = true;
        }

        OFConfig.checkDisplayMode();
        World world = this.mc.theWorld;

        if (world != null) {
            if (OFConfig.getNewRelease() != null) {
                String s = "HD_U".replace("HD_U", "HD Ultra").replace("L", "Light");
                String s1 = s + " " + OFConfig.getNewRelease();
                ...
                OFConfig.setNewRelease(null);
            }

            if (OFConfig.isNotify64BitJava()) {
                OFConfig.setNotify64BitJava(false);
        ...
        if (this.mc.currentScreen instanceof LettuceMainMenu) {
            this.updateMainMenu((LettuceMainMenu) this.mc.currentScreen);
        }
    ...
    private void frameFinish() {
        if (this.mc.theWorld != null && OFConfig.isShowGlErrors() && TimedEvent.isActive("CheckGlErrorFrameFinish", 10000L)) {
            int i = GlStateManager.glGetError();

            if (i != 0 && GlErrors.isEnabled(i)) {
                String s = OFConfig.getGlErrorString(i);
    ...
    private void updateMainMenu(LettuceMainMenu p_updateMainMenu_1_) {
        try {
            String s = null;
    ...
    private void loadAllVisibleChunks(Entity p_loadAllVisibleChunks_1_, double p_loadAllVisibleChunks_2_, ICamera p_loadAllVisibleChunks_4_, boolean p_loadAllVisibleChunks_5_) {
        ...
        try {
            ...
            RenderGlobal renderglobal = OFConfig.getRenderGlobal();
            int j = renderglobal.getCountLoadedChunks();
            long k = System.currentTimeMillis();
            OFConfig.dbg("Loading visible chunks");
            ...
            OFConfig.log("Chunks loaded: " + i1);
            OFConfig.log("Finished loading visible chunks");
