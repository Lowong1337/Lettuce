...
public class ScreenShotHelper
{
    ...
    public static IChatComponent saveScreenshot(File gameDirectory, String screenshotName, int width, int height, Framebuffer buffer)
    {
        try
        {
            ...
            int i = OFConfig.getGameSettings().guiScale;
            ScaledResolution scaledresolution = new ScaledResolution();
            int j = scaledresolution.getScaleFactor();
            int k = OFConfig.getScreenshotSize();
            boolean flag = OpenGlHelper.isFramebufferEnabled() && k > 1;

            if (flag)
            {
                OFConfig.getGameSettings().guiScale = j * k;
                resize(width * k, height * k);
                GlStateManager.pushMatrix();
                GlStateManager.clear(16640);
                minecraft.getFramebuffer().bindFramebuffer(true);
                minecraft.entityRenderer.updateCameraAndRender(OFConfig.renderPartialTicks, System.nanoTime());
            }
            ...
            if (flag)
            {
                ...
                OFConfig.getGameSettings().guiScale = i;
                resize(width, height);
            }
