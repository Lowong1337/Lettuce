...
public class IntegratedServer extends MinecraftServer {
    ...
    private void onTick(WorldServer p_onTick_1_) {
        if (!OFConfig.isTimeDefault()) {
            this.fixWorldTime(p_onTick_1_);
        }

        if (!OFConfig.isWeatherEnabled()) {
            this.fixWorldWeather(p_onTick_1_);
        }

        if (OFConfig.waterOpacityChanged) {
            OFConfig.waterOpacityChanged = false;
            ClearWater.updateWaterOpacity(OFConfig.getGameSettings(), p_onTick_1_);
        }
