...
public class TextureUtils
{
    ...
    {
        ITextureObject itextureobject = OFConfig.getTextureManager().getTexture(loc);

        if (itextureobject != null)
        {
            return itextureobject;
        }
        else if (!OFConfig.hasResource(loc))
        {
            return null;
        }
        else
        {
            ...
            OFConfig.getTextureManager().loadTexture(loc, simpletexture);
    ...
    public static void resourcesReloaded(IResourceManager rm)
    {
        ...
        {
            OFConfig.dbg("*** Reloading custom textures ***");
            CustomSky.reset();
            ...
            OFConfig.updateTexturePackClouds();
            ...
            OFConfig.getTextureManager().tick();
        }
    }
    ...
    {
        IResourceManager iresourcemanager = OFConfig.getResourceManager();
        ...
        OFConfig.getTextureManager().loadTickableTexture(resourcelocation, itickabletextureobject);
    }
    ...
    public static void applyAnisotropicLevel()
    {
        if (GLContext.getCapabilities().GL_EXT_texture_filter_anisotropic)
        {
            ...
            float f1 = (float) OFConfig.getAnisotropicFilterLevel();
            f1 = Math.min(f1, f);
    ...
    public static void dbgMipmaps(TextureAtlasSprite textureatlassprite)
    {
        ...
        for (int i = 0; i < aint.length; ++i)
        {
            ...
            if (aint1 == null)
            {
                OFConfig.dbg("" + i + ": " + aint1);
            }
            else
            {
                OFConfig.dbg("" + i + ": " + aint1.length);
            }
        }
    }
    ...
    public static void saveGlTexture(String name, int textureId, int mipmapLevels, int width, int height)
    {
        ...
        for (int i1 = 0; i1 <= mipmapLevels; ++i1)
        {
            ...
            try
            {
                ...
                OFConfig.dbg("Exported: " + file4);
            }
            catch (Exception exception)
            {
                OFConfig.warn("Error writing: " + file4);
                OFConfig.warn("" + exception.getClass().getName() + ": " + exception.getMessage());
            }
        }
    }
