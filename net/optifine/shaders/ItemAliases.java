...
public class ItemAliases
{
    ...
    public static void update(IShaderPack shaderPack)
    {
        ...
        if (shaderPack != null)
        {
            ...
            {
                OFConfig.dbg("[Shaders] Delayed loading of item mappings after resources are loaded");
    ...
    private static void loadItemAliases(InputStream in, String path, List<Integer> listItemAliases)
    {
        if (in != null)
        {
            try
            {
                ...
                properties.load(in);
                in.close();
                OFConfig.dbg("[Shaders] Parsing item mappings: " + path);
                ...
                for (Object o : properties.keySet())
                {
                    ...
                    if (!s.startsWith(s2))
                    {
                        OFConfig.warn("[Shaders] Invalid item ID: " + s);
                    }
                    else
                    {
                        ...
                        if (i < 0)
                        {
                            OFConfig.warn("[Shaders] Invalid item alias ID: " + i);
                        }
                        else
                        {
                            ...
                            {
                                OFConfig.warn("[Shaders] Invalid item ID mapping: " + s + "=" + s1);
                            }
                        }
                    }
                }
