...
public class BlockAliases
{
    ...
    public static void update(IShaderPack shaderPack)
    {
        ...
        if (shaderPack != null)
        {
            ...
            {
                OFConfig.dbg("[Shaders] Delayed loading of block mappings after resources are loaded");
    ...
    private static void loadBlockAliases(InputStream in, String path, List<List<BlockAlias>> listBlockAliases)
    {
        if (in != null)
        {
            try
            {
                ...
                properties.load(in);
                in.close();
                OFConfig.dbg("[Shaders] Parsing block mappings: " + path);
                ...
                for (Object o : properties.keySet())
                {
                    ...
                    else
                    {
                        ...
                        if (!s.startsWith(s2))
                        {
                            OFConfig.warn("[Shaders] Invalid block ID: " + s);
                        }
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    OFConfig.warn("[Shaders] Invalid block ID mapping: " + s + "=" + s1);
                                }
                            }
                        }
                    }
