...
public class Shaders
{
    ...
    private static void setEnumShaderOption(EnumShaderOption eso, String str)
    {
        ...
        {
            case ANTIALIASING:
                configAntialiasingLevel =OFConfig.parseInt(str, 0);
                break;

            case NORMAL_MAP:
                configNormalMap =OFConfig.parseBoolean(str, true);
                break;

            case SPECULAR_MAP:
                configSpecularMap =OFConfig.parseBoolean(str, true);
                break;

            case RENDER_RES_MUL:
                configRenderResMul =OFConfig.parseFloat(str, 1.0F);
                break;

            case SHADOW_RES_MUL:
                configShadowResMul =OFConfig.parseFloat(str, 1.0F);
                break;

            case HAND_DEPTH_MUL:
                configHandDepthMul =OFConfig.parseFloat(str, 0.125F);
                break;

            case CLOUD_SHADOW:
                configCloudShadow =OFConfig.parseBoolean(str, true);
                break;
            ...
            case TWEAK_BLOCK_DAMAGE:
                configTweakBlockDamage =OFConfig.parseBoolean(str, true);
                break;

            case SHADOW_CLIP_FRUSTRUM:
                configShadowClipFrustrum =OFConfig.parseBoolean(str, true);
                break;

            case TEX_MIN_FIL_B:
                configTexMinFilB =OFConfig.parseInt(str, 0);
                break;

            case TEX_MIN_FIL_N:
                configTexMinFilN =OFConfig.parseInt(str, 0);
                break;

            case TEX_MIN_FIL_S:
                configTexMinFilS =OFConfig.parseInt(str, 0);
                break;

            case TEX_MAG_FIL_B:
                configTexMagFilB =OFConfig.parseInt(str, 0);
                break;
    ...
    private static void loadShaderPackDimensions()
    {
        ...
        if (shaderPackDimensions.size() > 0)
        {
           ...
           OFConfig.dbg("[Shaders] Worlds: " +OFConfig.arrayToString((Object[])ainteger));
        }
    }
    ...
    public static ShaderOption[] getShaderPackOptions(String screenName)
    {
        ...
        if (shaderPackGuiScreens == null)
        {
            if (shaderPackProfiles != null)
            {
                ...
                ashaderoption = (ShaderOption[])((ShaderOption[])OFConfig.addObjectToArray(ashaderoption, shaderoptionprofile, 0));
            }
