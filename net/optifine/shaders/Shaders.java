...
public class Shaders {
    ...
    private static void setEnumShaderOption(EnumShaderOption eso, String str) {
        ...
        switch (eso) {
            case ANTIALIASING -> configAntialiasingLevel = OFConfig.parseInt(str, 0);
            case NORMAL_MAP -> configNormalMap = OFConfig.parseBoolean(str, true);
            case SPECULAR_MAP -> configSpecularMap = OFConfig.parseBoolean(str, true);
            case RENDER_RES_MUL -> configRenderResMul = OFConfig.parseFloat(str, 1.0F);
            case SHADOW_RES_MUL -> configShadowResMul = OFConfig.parseFloat(str, 1.0F);
            case HAND_DEPTH_MUL -> configHandDepthMul = OFConfig.parseFloat(str, 0.125F);
            case CLOUD_SHADOW -> configCloudShadow = OFConfig.parseBoolean(str, true);
            case OLD_HAND_LIGHT -> configOldHandLight.setPropertyValue(str);
            case OLD_LIGHTING -> configOldLighting.setPropertyValue(str);
            case SHADER_PACK -> currentShaderName = str;
            case TWEAK_BLOCK_DAMAGE -> configTweakBlockDamage = OFConfig.parseBoolean(str, true);
            case SHADOW_CLIP_FRUSTRUM -> configShadowClipFrustrum = OFConfig.parseBoolean(str, true);
            case TEX_MIN_FIL_B -> configTexMinFilB = OFConfig.parseInt(str, 0);
            case TEX_MIN_FIL_N -> configTexMinFilN = OFConfig.parseInt(str, 0);
            case TEX_MIN_FIL_S -> configTexMinFilS = OFConfig.parseInt(str, 0);
            case TEX_MAG_FIL_B -> configTexMagFilB = OFConfig.parseInt(str, 0);
            case TEX_MAG_FIL_N -> configTexMagFilB = OFConfig.parseInt(str, 0);
            case TEX_MAG_FIL_S -> configTexMagFilB = OFConfig.parseInt(str, 0);
    ...
    public static void loadShaderPack() {
        ...
        if (OFConfig.isAntialiasing()) {
            SMCLog.info("Shaders can not be loaded, Antialiasing is enabled: " + OFConfig.getAntialiasingLevel() + "x");
            flag2 = true;
        }

        if (OFConfig.isAnisotropicFiltering()) {
            SMCLog.info("Shaders can not be loaded, Anisotropic Filtering is enabled: " + OFConfig.getAnisotropicFilterLevel() + "x");
            flag2 = true;
        }
    ...
    private static void loadShaderPackDimensions() {
        ...
        if (shaderPackDimensions.size() > 0) {
            ...
            OFConfig.dbg("[Shaders] Worlds: " + OFConfig.arrayToString(ainteger));
        }
    }
    ...
    private static void loadShaderPackProperties() {
        ...
        if (shaderPack != null) {
            ...
            try {
                ...
                OFConfig.warn("[Shaders] Error reading: " + s);
            }
        }
    }
    ...
    private static ICustomTexture loadCustomTextureRaw(int textureUnit, String line) {
        ...
        String[] astring = OFConfig.tokenize(line, " ");
        ...
        if (texturetype == null) {
            ...
            if (internalformat == null) {
                ...
                switch (texturetype) {
                    case TEXTURE_1D -> i = connectedparser.parseInt(deque.poll(), -1);
    ...
    private static ICustomTexture loadCustomTextureRaw(int textureUnit, String line, String path, TextureType type, InternalFormat internalFormat, int width, int height, int depth, PixelFormat pixelFormat, PixelType pixelType) {
        try {
            ...
            if (inputstream == null) {
                ...
                byte[] abyte = OFConfig.readAll(inputstream);
    ...
    public static ShaderOption[] getShaderPackOptions(String screenName) {
        ...
        if (shaderPackGuiScreens == null) {
            if (shaderPackProfiles != null) {
                ...
                ashaderoption = (ShaderOption[]) OFConfig.addObjectToArray(ashaderoption, shaderoptionprofile, 0);
            }
    ...
    private static void saveShaderPackOptions(ShaderOption[] sos, IShaderPack sp) {
        ...
        try {
            ...
            OFConfig.warn("[Shaders] Error saving configuration for " + shaderPack.getName());
    ...
    private static ShaderOption[] loadShaderPackOptions() {
        try {
            ...
            for (int i = 0; i < ashaderoption.length; ++i) {
                ...
                if (s != null) {
                    ...
                    if (!shaderoption.setValue(s)) {
                        OFConfig.warn("[Shaders] Invalid value, option: " + shaderoption.getName() + ", value: " + s);
                    }
                }
            }
            ...
            OFConfig.warn("[Shaders] Error reading configuration for " + shaderPack.getName());
            ioexception.printStackTrace();
            return null;
        }
    }
    ...
    public static int checkGLError(String location) {
        ...
        if (i != 0 && GlErrors.isEnabled(i)) {
            ...
            if (OFConfig.isShowGlErrors() && TimedEvent.isActive("ShowGlErrorShaders", 10000L)) {
                        ...
                        if (macrostate.processLine(s)) {
                            ...
                            if (shaderline != null) {
                                if (shaderline.isExtension("GL_ARB_geometry_shader4")) {
                                    String s1 = OFConfig.normalize(shaderline.getValue());
                                            ...
                                            if (shaderline.isConstFloat("shadowDistanceRenderMul")) {
                                                ...
                                                if (!shaderline.isConstFloat("drynessHalflife") && !shaderline.isProperty("DRYNESSHL")) {
                                                    if (shaderline.isConstFloat("eyeBrightnessHalflife")) {
                                                        ...
                                                        aoLevel = OFConfig.limit(shaderline.getValueFloat(), 0.0F, 1.0F);
    ...
    public static void saveShader(String filename, String code) {
        try {
            ...
            OFConfig.writeFile(file1, code);
        } catch (IOException ioexception) {
            OFConfig.warn("Error saving: " + filename);
            ioexception.printStackTrace();
        }
    }
    ...
    public static void beginRender(Minecraft minecraft, float partialTicks, long finishTimeNano) {
        ...
        if (!isShaderPackInitialized) {
            try {
                ...
                if (OFConfig.normalize(illegalstateexception.getMessage()).equals("Function is not supported")) {
        ...
        if (entity != null) {
            ...
            if (mc.thePlayer != null) {
                ...
                if (mc.thePlayer.isPotionActive(Potion.nightVision)) {
                    nightVision = OFConfig.getMinecraft().entityRenderer.getNightVisionBrightness(mc.thePlayer, partialTicks);
                }
                ...
                if (mc.thePlayer.isPotionActive(Potion.blindness)) {
                    ...
                    blindness = OFConfig.limit((float) i / 20.0F, 0.0F, 1.0F);
                }
            }
    ...
    public static void nextAntialiasingLevel(boolean forward) {
        ...
        configAntialiasingLevel = OFConfig.limit(configAntialiasingLevel, 0, 4);
    }
    ...
    private static void loadShaderPackResources() {
        ...
        if (shaderPackLoaded) {
            ...
            if (!OFConfig.getGameSettings().language.equals(s1)) {
                list.add(s + OFConfig.getGameSettings().language + s2);
            }
