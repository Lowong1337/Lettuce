...
public class EntityAliases
{
    ...
    public static void update(IShaderPack shaderPack)
    {
        ...
        if (shaderPack != null)
        {
            if (Reflector.Loader_getActiveModList.exists() && OFConfig.getResourceManager() == null)
            {
                OFConfig.dbg("[Shaders] Delayed loading of entity mappings after resources are loaded");
    ...
    private static void loadEntityAliases(InputStream in, String path, List<Integer> listEntityAliases)
    {
        if (in != null)
        {
            try
            {
                ...
                properties.load(in);
                in.close();
                OFConfig.dbg("[Shaders] Parsing entity mappings: " + path);
                ...
                for (Object o : properties.keySet())
                {
                    ...
                    if (!s.startsWith(s2))
                    {
                        OFConfig.warn("[Shaders] Invalid entity ID: " + s);
                    }
                    else
                    {
                        String s3 = StrUtils.removePrefix(s, s2);
                        int i = OFConfig.parseInt(s3, -1);

                        if (i < 0)
                        {
                            OFConfig.warn("[Shaders] Invalid entity alias ID: " + i);
                        }
                        else
                        {
                            ...
                            {
                                OFConfig.warn("[Shaders] Invalid entity ID mapping: " + s + "=" + s1);
                            }
                        }
                    }
                }
            ...
            {
                OFConfig.warn("[Shaders] Error reading: " + path);
            }
        }
    }
