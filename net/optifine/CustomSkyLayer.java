...
public class CustomSkyLayer
{
    ...
    private List<String> parseWeatherList(String str)
    {
        ...
        for (int i = 0; i < astring.length; ++i)
        {
            ...
            if (!list.contains(s))
            {
                OFConfig.warn("Unknown weather: " + s);
            }
            else
            {
                list1.add(s);
    ...
    private int parseTime(String str)
    {
        ...
        else
        {
            ...
            if (astring.length != 2)
            {
                OFConfig.warn("Invalid time: " + str);
                return -1;
            }
            else
            {
                String s = astring[0];
                String s1 = astring[1];
                int i = OFConfig.parseInt(s, -1);
                int j = OFConfig.parseInt(s1, -1);
    ...
    private boolean parseBoolean(String str, boolean defVal)
    {
        ...
        {
            OFConfig.warn("Unknown boolean: " + str);
            return defVal;
        }
    }
    ...
    private float parseFloat(String str, float defVal)
    {
        ...
        else
        {
            ...
            {
                OFConfig.warn("Invalid value: " + str);
                return defVal;
            }
            else
            {
    ...
    private float[] parseAxis(String str, float[] defVal)
    {
        ...
        else
        {
            ...
            if (astring.length != 3)
            {
                OFConfig.warn("Invalid axis: " + str);
                return defVal;
            }
            else
            {
                ...
                for (int i = 0; i < astring.length; ++i)
                {
                    afloat[i] = OFConfig.parseFloat(astring[i], Float.MIN_VALUE);
                    ...
                    {
                        OFConfig.warn("Invalid axis values: " + str);
                        return defVal;
                    }
                }
    ...
    public boolean isValid(String path)
    {
        if (this.source == null)
        {
            OFConfig.warn("No source texture: " + path);
            return false;
        }
        else
        {
            ...
            if (this.startFadeIn >= 0 && this.endFadeIn >= 0 && this.endFadeOut >= 0)
            {
                ...
                if (i1 != 24000)
                {
                    OFConfig.warn("Invalid fadeIn/fadeOut times, sum is not 24h: " + i1);
                    return false;
                }
                else if (this.speed < 0.0F)
                {
                    OFConfig.warn("Invalid speed: " + this.speed);
                    return false;
                }
                else if (this.daysLoop <= 0)
                {
                    OFConfig.warn("Invalid daysLoop: " + this.daysLoop);
                    return false;
                }
                else
                {
            ...
            {
                OFConfig.warn("Invalid times, required are: startFadeIn, endFadeIn and endFadeOut.");
                return false;
            }
        }
    }
    ...
    public void render(World world, int timeOfDay, float celestialAngle, float rainStrength, float thunderStrength)
    {
        ...
        float f3 = f * f1 * f2;
        f3 = OFConfig.limit(f3, 0.0F, 1.0F);
