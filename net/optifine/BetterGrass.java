...
public class BetterGrass
{
    ...
    private static void loadProperties(TextureMap textureMap)
    {
        ...
        try
        {
            ...
            if (flag)
            {
                OFConfig.dbg("BetterGrass: Parsing default configuration " + s);
            }
            else
            {
                OFConfig.dbg("BetterGrass: Parsing configuration " + s);
            }
        ...
        {
            OFConfig.warn("Error reading: " + s + ", " + ioexception.getClass().getName() + ": " + ioexception.getMessage());
        }
    }
    ...
    private static TextureAtlasSprite registerSprite(Properties props, String key, String textureDefault, TextureMap textureMap)
    {
        ...
        if (!OFConfig.hasResource(resourcelocation))
        {
            OFConfig.warn("BetterGrass texture not found: " + resourcelocation);
            s = textureDefault;
        }
    ...
    private static List getFaceQuadsMycelium(IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, EnumFacing facing, List quads)
    {
        ...
        if (OFConfig.isBetterGrassFancy())
        {
            if (flag)
            {
