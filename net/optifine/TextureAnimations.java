...
public class TextureAnimations
{
    ...
    {
        if (textureAnimations != null && OFConfig.isAnimatedTextures())
        {
            int i = 0;
            ...
            int k = OFConfig.getMinecraft().entityRenderer.frameCount;
    ...
    private static TextureAnimation[] getTextureAnimations(IResourcePack rp)
    {
        ...
        else
        {
            ...
            {
                String s = astring[i];
                OFConfig.dbg("Texture animation: " + s);
                ...
                try
                {
                    ...
                    if (textureanimation != null)
                    {
                        ...
                        if (OFConfig.getDefiningResourcePack(resourcelocation1) != rp)
                        {
                            OFConfig.dbg("Skipped: " + s + ", target texture not loaded from same resource pack");
                        }
                        else
                        {
                ...
                {
                    OFConfig.warn("File not found: " + filenotfoundexception.getMessage());
                }
    ...
    private static TextureAnimation makeTextureAnimation(Properties props, ResourceLocation propLoc)
    {
        ...
        int i = OFConfig.parseInt(props.getProperty("x"), -1);
        int j = OFConfig.parseInt(props.getProperty("y"), -1);
        int k = OFConfig.parseInt(props.getProperty("w"), -1);
        int l = OFConfig.parseInt(props.getProperty("h"), -1);
        ...
        if (s != null && s1 != null)
        {
            if (i >= 0 && j >= 0 && k >= 0 && l >= 0)
            {
                ...
                if (abyte == null)
                {
                    OFConfig.warn("TextureAnimation: Source texture not found: " + s1);
                    return null;
                }
                else
                {
                    ...
                    if (i1 != k1)
                    {
                        OFConfig.warn("TextureAnimation: Source texture has invalid number of frames: " + s + ", frames: " + (float)i1 / (float)(k * l));
                        return null;
                    }
                    else
                    {
                        ...
                        try
                        {
                            ...
                            if (inputstream == null)
                            {
                                OFConfig.warn("TextureAnimation: Target texture not found: " + s1);
                                return null;
                            }
                            else
                            {
                                ...
                                {
                                    OFConfig.warn("TextureAnimation: Animation coordinates are outside the target texture: " + s1);
                                    return null;
                                }
                            }
                        }
            ...
            {
                OFConfig.warn("TextureAnimation: Invalid coordinates");
                return null;
            }
        }
        else
        {
            OFConfig.warn("TextureAnimation: Source or target texture not specified");
            return null;
        }
    }
