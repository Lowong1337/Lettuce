...
public class CustomColors
{
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (!list.contains(s))
                    {
                        ...
                        warn("Expected values: " + OFConfig.arrayToString((Object[])validValues));
                        return valDef;
                    }
                    else
                    {
    ...
    private static Pair<LightMapPack[], Integer> parseLightMapPacks()
    {
        ...
        for (int i = 0; i < astring.length; ++i)
        {
            ...
            int j = OFConfig.parseInt(s3, Integer.MIN_VALUE);
    ...
    private static int getTextureHeight(String path, int defHeight)
    {
        try
        {
            InputStream inputstream = OFConfig.getResourceStream(new ResourceLocation(path));
    ...
    private static void readColorProperties(String fileName)
    {
        try
        {
            ...
            xpOrbTime = OFConfig.parseInt(properties.getProperty("xporb.time"), -1);
        }
    ...
    private static void addToBlockList(CustomColormap cm, List blockList)
    {
        ...
        {
            warn("No match blocks: " + OFConfig.arrayToString(aint));
        }
    }
    ...
    public static CustomColormap getCustomColors(String pathImage, int width, int height)
    {
        try
        {
            ...
            else
            {
                ...
                {
                    InputStream inputstream = OFConfig.getResourceStream(resourcelocation1);
                    properties.load(inputstream);
                    inputstream.close();
    ...
    {
        useDefaultGrassFoliageColors = foliageBirchColors == null && foliagePineColors == null && swampGrassColors == null && swampFoliageColors == null && OFConfig.isSwampColors() && OFConfig.isSmoothBiomes();
    }
    ...
    public static int getColorMultiplier(BakedQuad quad, IBlockState blockState, IBlockAccess blockAccess, BlockPos blockPos, RenderEnv renderEnv)
    {
        ...
        if (blockColormaps != null)
        {
            ...
            if (customcolormap != null)
            {
                if (OFConfig.isSmoothBiomes() && !customcolormap.isColorConstant())
                {
        ...
        else
        {
            ...
            return OFConfig.isSmoothBiomes() && !customcolors$icolorizer.isColorConstant() ? getSmoothColorMultiplier(blockState, blockAccess, blockPos, customcolors$icolorizer, renderEnv.getColorizerBlockPosM()) : customcolors$icolorizer.getColor(iblockstate, blockAccess, blockPos);
        }
    }
    ...
    protected static BiomeGenBase getColorBiome(IBlockAccess blockAccess, BlockPos blockPos)
    {
        ...
        if (biomegenbase == BiomeGenBase.swampland && !OFConfig.isSwampColors())
        {
    ...
    {
        OFConfig.dbg("CustomColors: " + str);
    }

    private static void warn(String str)
    {
        OFConfig.warn("CustomColors: " + str);
    }
