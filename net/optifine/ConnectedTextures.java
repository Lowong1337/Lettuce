...
public class ConnectedTextures
{
    ...
    private static TextureAtlasSprite getConnectedTextureRandom(ConnectedProperties cp, IBlockAccess blockAccess, BlockStateBase blockState, BlockPos blockPos, int side)
    {
        ...
        else
        {
            ...
            int l = OFConfig.getRandom(blockPos, i) & Integer.MAX_VALUE;

            for (int i1 = 0; i1 < cp.randomLoops; ++i1)
            {
                l = OFConfig.intHash(l);
            }
    ...
    private static int getConnectedTextureCtmIndex(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, int vertAxis, int side, TextureAtlasSprite icon, int metadata, RenderEnv renderEnv)
    {
        ...
        else if (!OFConfig.isConnectedTexturesFancy())
        {
            return i;
        }
        else
    ...
    private static boolean isNeighbourOverlay(ConnectedProperties cp, IBlockAccess iblockaccess, IBlockState blockState, BlockPos blockPos, int side, TextureAtlasSprite icon, int metadata)
    {
        ...
        else
        {
            ...
            if (cp.connectTileIcons != null)
            {
                ...
                if (!OFConfig.isSameOne(textureatlassprite, cp.connectTileIcons))
                {
                    return false;
                }
            }
    ...
    public static void updateIcons(TextureMap textureMap)
    {
        ...
        if (OFConfig.isConnectedTextures())
        {
            IResourcePack[] airesourcepack = OFConfig.getResourcePacks();
            ...
            updateIcons(textureMap, OFConfig.getDefaultResourcePack());
    ...
    public static void updateIcons(TextureMap textureMap, IResourcePack rp)
    {
        ...
        {
            String s = astring[i];
            OFConfig.dbg("ConnectedTextures: " + s);
            ...
            try
            {
                ...
                if (inputstream == null)
                {
                    OFConfig.warn("ConnectedTextures file not found: " + s);
                }
                else
                {
        ...
        OFConfig.dbg("Multipass connected textures: " + multipass);
    }
    ...
    private static void addToTileList(ConnectedProperties cp, List tileList)
    {
        if (cp.matchTileIcons != null)
        {
            for (int i = 0; i < cp.matchTileIcons.length; ++i)
            {
                ...
                {
                    OFConfig.warn("TextureAtlasSprite is not TextureAtlasSprite: " + textureatlassprite + ", name: " + textureatlassprite.getIconName());
                }
                else
                {
                    ...
                    if (j < 0)
                    {
                        OFConfig.warn("Invalid tile ID: " + j + ", icon: " + textureatlassprite.getIconName());
                    }
                    else
                    {
    ...
    private static void addToBlockList(ConnectedProperties cp, List blockList)
    {
        if (cp.matchBlocks != null)
        {
            for (int i = 0; i < cp.matchBlocks.length; ++i)
            {
                ...
                if (j < 0)
                {
                    OFConfig.warn("Invalid block ID: " + j);
                }
                else
                {
    ...
    private static String[] getDefaultCtmPaths()
    {
        ...
        if (OFConfig.isFromDefaultResourcePack(new ResourceLocation("textures/blocks/glass.png")))
        {
            list.add(s + "glass.properties");
            list.add(s + "glasspane.properties");
        }

        if (OFConfig.isFromDefaultResourcePack(new ResourceLocation("textures/blocks/bookshelf.png")))
        {
            list.add(s + "bookshelf.properties");
        }

        if (OFConfig.isFromDefaultResourcePack(new ResourceLocation("textures/blocks/sandstone_normal.png")))
        {
        ...
        for (int i = 0; i < astring.length; ++i)
        {
            ...
            if (OFConfig.isFromDefaultResourcePack(new ResourceLocation("textures/blocks/glass_" + s1 + ".png")))
            {
