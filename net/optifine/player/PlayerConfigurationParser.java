...
public class PlayerConfigurationParser
{
            ...
            if (jsonarray != null)
            {
                for (int i = 0; i < jsonarray.size(); ++i)
                {
                    ...
                    if (flag)
                    {
                        ...
                        if (s == null)
                        {
                            OFConfig.warn("Item type is null, player: " + this.player);
                        }
                        else
                        {
    ...
    private BufferedImage downloadTextureImage(String texturePath)
    {
        ...
        {
            OFConfig.warn("Error loading item texture " + texturePath + ": " + ioexception.getClass().getName() + ": " + ioexception.getMessage());
            return null;
        }
    }
    ...
    private PlayerItemModel downloadModel(String modelPath)
    {
        ...
        {
            OFConfig.warn("Error loading item model " + modelPath + ": " + exception.getClass().getName() + ": " + exception.getMessage());
            return null;
        }
    }
}
