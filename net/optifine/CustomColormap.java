...
public class CustomColormap implements CustomColors.IColorizer
{
    ...
    {
        index = OFConfig.limit(index, 0, this.colors.length - 1);
        return this.colors[index] & 16777215;
    }

    public int getColor(int cx, int cy)
    {
        cx = OFConfig.limit(cx, 0, this.width - 1);
        cy = OFConfig.limit(cy, 0, this.height - 1);
    ...
    public void addMatchBlock(MatchBlock mb)
    {
        ...
        this.matchBlocks = (MatchBlock[])((MatchBlock[]) OFConfig.addObjectToArray(this.matchBlocks, mb));
    }
    ...
    {
        return "" + this.basePath + "/" + this.name + ", blocks: " + OFConfig.arrayToString((Object[])this.matchBlocks) + ", source: " + this.source;
    }
}
