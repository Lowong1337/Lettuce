...
public class ConnectedProperties
{
                    ...
                    if (s.startsWith(s1))
                    {
                        ...
                        if (s3 != null)
                        {
                            ...
                            if (i >= 0 && i <= 46)
                            {
                                ...
                                {
                                    OFConfig.warn("Invalid CTM tile index: " + s3);
                                }
                            }
                            else
                            {
                                OFConfig.warn("Invalid CTM index: " + s2);
                            }
                        }
                    }
                }
    ...
    private String[] parseMatchTiles(String str)
    {
        ...
        {
            String[] astring = OFConfig.tokenize(str, " ");
    ...
    private String[] parseTileNames(String str)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < astring.length; ++i)
            {
                ...
                if (s.contains("-"))
                {
                    String[] astring1 = OFConfig.tokenize(s, "-");

                    if (astring1.length == 2)
                    {
                        int j = OFConfig.parseInt(astring1[0], -1);
                        int k = OFConfig.parseInt(astring1[1], -1);

                        if (j >= 0 && k >= 0)
                        {
                            if (j > k)
                            {
                                OFConfig.warn("Invalid interval: " + s + ", when parsing: " + str);
                                continue;
                            }
    ...
    private static int parseSymmetry(String str)
    {
        ...
        else
        {
            ...
            {
                OFConfig.warn("Unknown symmetry: " + str);
                return 1;
            }
        }
    }
    ...
    private static int parseFace(String str)
    {
        ...
        if (!str.equals("bottom") && !str.equals("down"))
        {
            if (!str.equals("top") && !str.equals("up"))
            {
                ...
                {
                    OFConfig.warn("Unknown face: " + str);
                    return 128;
                }
            }
            else
    ...
    private static int parseConnect(String str)
    {
        ...
        else
        {
            ...
            {
                OFConfig.warn("Unknown connect: " + str);
                return 128;
            }
        }
    }
                ...
                else if (!str.equals("horizontal") && !str.equals("bookshelf"))
                {
                    ...
                    else if (!str.equals("horizontal+vertical") && !str.equals("h+v"))
                    {
                        if (!str.equals("vertical+horizontal") && !str.equals("v+h"))
                        {
                            ...
                            {
                                OFConfig.warn("Unknown method: " + str);
                                return 0;
                            }
                        }
                        else
    ...
    public boolean isValid(String path)
    {
        if (this.name != null && this.name.length() > 0)
        {
            ...
            {
                OFConfig.warn("No base path found: " + path);
                return false;
            }
            else
            {
                ...
                {
                    OFConfig.warn("No matchBlocks or matchTiles specified: " + path);
                    return false;
                }
                else if (this.method == 0)
                {
                    OFConfig.warn("No method: " + path);
                    return false;
                }
                else if (this.tiles != null && this.tiles.length > 0)
                {
                    ...
                    if (this.connect == 128)
                    {
                        OFConfig.warn("Invalid connect in: " + path);
                        return false;
                    }
                    else if (this.renderPass > 0)
                    {
                        OFConfig.warn("Render pass not supported: " + this.renderPass);
                        return false;
                    }
                    else if ((this.faces & 128) != 0)
                    {
                        OFConfig.warn("Invalid faces in: " + path);
                        return false;
                    }
                    else if ((this.symmetry & 128) != 0)
                    {
                        OFConfig.warn("Invalid symmetry in: " + path);
                        return false;
                    }
                    else
                    {
                        switch (this.method)
                        {
                            ...
                            default:
                                OFConfig.warn("Unknown method: " + path);
                                return false;
                        }
                    }
                }
                ...
                {
                    OFConfig.warn("No tiles specified: " + path);
                    return false;
                }
            }
        }
        ...
        {
            OFConfig.warn("No name found: " + path);
            return false;
        }
    }
    ...
    private boolean isValidCtm(String path)
    {
        ...
        {
            OFConfig.warn("Invalid tiles, must be at least 47: " + path);
            return false;
        }
        else
        {
    ...
    private boolean isValidCtmCompact(String path)
    {
        ...
        {
            OFConfig.warn("Invalid tiles, must be at least 5: " + path);
            return false;
        }
        else
        {
    ...
    private boolean isValidOverlay(String path)
    {
        ...
        {
            OFConfig.warn("Invalid tiles, must be at least 17: " + path);
            return false;
        }
        ...
        {
            OFConfig.warn("Invalid overlay layer: " + this.layer);
            return false;
        }
    }
    ...
    private boolean isValidHorizontal(String path)
    {
        ...
        {
            OFConfig.warn("Invalid tiles, must be exactly 4: " + path);
            return false;
        }
        else
        {
    ...
    private boolean isValidVertical(String path)
    {
        if (this.tiles == null)
        {
            OFConfig.warn("No tiles defined for vertical: " + path);
            return false;
        }
    ...
    private boolean isValidHorizontalVertical(String path)
    {
        if (this.tiles == null)
        {
            OFConfig.warn("No tiles defined for horizontal+vertical: " + path);
            return false;
        }
        else if (this.tiles.length != 7)
        {
            OFConfig.warn("Invalid tiles, must be exactly 7: " + path);
            return false;
        }
        else
        {
    ...
    private boolean isValidVerticalHorizontal(String path)
    {
        if (this.tiles == null)
        {
            OFConfig.warn("No tiles defined for vertical+horizontal: " + path);
            return false;
        }
        else if (this.tiles.length != 7)
        {
    ...
    private boolean isValidRandom(String path)
    {
        if (this.tiles != null && this.tiles.length > 0)
        {
            if (this.weights != null)
            {
                ...
                {
                    OFConfig.warn("More weights defined than tiles, trimming weights: " + path);
                ...
                if (this.weights.length < this.tiles.length)
                {
                    OFConfig.warn("Less weights defined than tiles, expanding weights: " + path);
                ...
                {
                    OFConfig.warn("Invalid sum of all weights: " + k);
            ...
            {
                OFConfig.warn("Invalid randomLoops: " + this.randomLoops);
                return false;
            }
        }
        else
        {
            OFConfig.warn("Tiles not defined: " + path);
            return false;
        }
    }
    ...
    private boolean isValidRepeat(String path)
    {
        ...
        {
            OFConfig.warn("Invalid width: " + path);
            return false;
        }
        else if (this.height <= 0)
        {
            OFConfig.warn("Invalid height: " + path);
            return false;
        }
        else if (this.tiles.length != this.width * this.height)
        {
            OFConfig.warn("Number of tiles does not equal width x height: " + path);
            return false;
        }
        else
        {
    ...
    private boolean isValidFixed(String path)
    {
        ...
        {
            OFConfig.warn("Number of tiles should be 1 for method: fixed.");
            return false;
        }
        else
        {
    ...
    private boolean isValidTop(String path)
    {
        ...
        {
            OFConfig.warn("Invalid tiles, must be exactly 1: " + path);
            return false;
        }
        else
        {
        ...
        else
        {
            ...
            for (int i = 0; i < tileNames.length; ++i)
            {
                ...
                else
                {
                    ...
                    boolean flag = OFConfig.hasResource(resourcelocation1);

                    if (!flag)
                    {
                        OFConfig.warn("File not found: " + s3);
                    }
    ...
    {
        return "CTM name: " + this.name + ", basePath: " + this.basePath + ", matchBlocks: " + OFConfig.arrayToString((Object[])this.matchBlocks) + ", matchTiles: " + OFConfig.arrayToString((Object[])this.matchTiles);
    }
