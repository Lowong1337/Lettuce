...
public class RandomEntityRule
{
    ...
    private RangeListInt parseMinMaxHeight(Properties props, int index)
    {
        ...
        else
        {
            ...
            if (s != null)
            {
                i = OFConfig.parseInt(s, -1);

                if (i < 0)
                {
                    OFConfig.warn("Invalid minHeight: " + s);
                    return null;
                }
            }
            ...
            if (s1 != null)
            {
                j = OFConfig.parseInt(s1, -1);

                if (j < 0)
                {
                    OFConfig.warn("Invalid maxHeight: " + s1);
                    return null;
                }
            }
            ...
            if (j < 0)
            {
                OFConfig.warn("Invalid minHeight, maxHeight: " + s + ", " + s1);
                return null;
            }
            else
            {
    ...
    public boolean isValid(String path)
    {
        if (this.textures != null && this.textures.length != 0)
        {
            ...
            else
            {
                ...
                {
                    OFConfig.warn("Invalid path: " + this.baseResLoc.getResourcePath());
                    return false;
                }
                else
                {
                    for (int i = 0; i < this.resourceLocations.length; ++i)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                OFConfig.warn("Texture not found: " + resourcelocation1.getResourcePath());
                                return false;
                            }
                    ...
                    if (this.weights != null)
                    {
                        ...
                        {
                            OFConfig.warn("More weights defined than skins, trimming weights: " + path);
                        ...
                        if (this.weights.length < this.resourceLocations.length)
                        {
                            OFConfig.warn("Less weights defined than skins, expanding weights: " + path);
                        ...
                        for (int i1 = 0; i1 < this.weights.length; ++i1)
                        {
                            ...
                            {
                                OFConfig.warn("Invalid weight: " + this.weights[i1]);
                                return false;
                            }
                    ...
                    {
                        OFConfig.warn("Invalid professions or careers: " + path);
                        return false;
                    }
                    else if (this.collarColors == ConnectedParser.DYE_COLORS_INVALID)
                    {
                        OFConfig.warn("Invalid collar colors: " + path);
                        return false;
                    }
                    else
                    {
        ...
        {
            OFConfig.warn("Invalid skins for rule: " + this.index);
            return false;
        }
    }
        ...
        else
        {
            ...
            if (this.collarColors != null && randomEntity instanceof RandomEntity)
            {
                ...
                if (entity1 instanceof EntityWolf)
                {
                    ...
                    if (!OFConfig.equalsOne(enumdyecolor, this.collarColors))
                    {
                        return false;
                    }
                }
            ...
            {
                World world = OFConfig.getMinecraft().theWorld;
            ...
            {
                World world1 = OFConfig.getMinecraft().theWorld;
            ...
            {
                World world2 = OFConfig.getMinecraft().theWorld;
