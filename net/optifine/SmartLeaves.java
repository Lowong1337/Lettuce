...
public class SmartLeaves
{
    ...
    public static void updateLeavesModels()
    {
        ...
        if (list.size() > 0)
        {
            OFConfig.dbg("Enable face culling: " + OFConfig.arrayToString(list.toArray()));
        }
    }
    ...
    static IBakedModel getModelCull(String type, List updatedTypes)
    {
        ...
        else
        {
            ...
            if (OFConfig.getDefiningResourcePack(resourcelocation) != OFConfig.getDefaultResourcePack())
            {
                return null;
            }
            else
            {
                ...
                if (OFConfig.getDefiningResourcePack(resourcelocation1) != OFConfig.getDefaultResourcePack())
                {
                    return null;
                }
                else
    ...
    private static IBakedModel getModelDoubleFace(IBakedModel model)
    {
        ...
        {
            OFConfig.warn("SmartLeaves: Model is not cube, general quads: " + model.getGeneralQuads().size() + ", model: " + model);
            return model;
        }
        else
        {
            ...
            for (int i = 0; i < aenumfacing.length; ++i)
            {
                ...
                if (list.size() != 1)
                {
                    OFConfig.warn("SmartLeaves: Model is not cube, side: " + enumfacing + ", quads: " + list.size() + ", model: " + model);
                    return model;
                }
            }
