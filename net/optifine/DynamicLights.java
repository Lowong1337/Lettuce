...
public class DynamicLights
{
    ...
    private static void initialize()
    {
        ...
        {
            OFConfig.dbg("DynamicLights entities: " + mapEntityLightLevels.size());
        }

        if (mapItemLightLevels.size() > 0)
        {
            OFConfig.dbg("DynamicLights items: " + mapItemLightLevels.size());
        }
    }
    ...
    private static void loadModConfiguration(InputStream in, String path, String modId)
    {
        if (in != null)
        {
            try
            {
                ...
                properties.load(in);
                in.close();
                OFConfig.dbg("DynamicLights: Parsing " + path);
            ...
            {
                OFConfig.warn("DynamicLights: Error reading " + path);
            }
        }
    }
    ...
    private static void loadModLightLevels(String prop, Map mapLightLevels, IObjectLocator ol, ConnectedParser cp, String path, String modId)
    {
        if (prop != null)
        {
            String[] astring = OFConfig.tokenize(prop, " ");
    ...
    public static double getLightLevel(BlockPos pos)
    {
        ...
        synchronized (mapDynamicLights)
        {
            ...
            for (int j = 0; j < i; ++j)
            {
                ...
                if (k > 0)
                {
                    ...
                    if (dynamiclight.isUnderwater() && !OFConfig.isClearWater())
                    {
                        k = OFConfig.limit(k - 2, 0, 15);
                        d7 *= 2.0D;
                    }
        ...
        double d11 = OFConfig.limit(d0, 0.0D, 15.0D);
        return d11;
    }
    ...
    {
        if (entity == OFConfig.getMinecraft().getRenderViewEntity() && !OFConfig.isDynamicHandLight())
        {
            return 0;
        }
        else
