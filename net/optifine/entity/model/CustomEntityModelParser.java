...
public class CustomEntityModelParser
{
    ...
    private static void processBaseId(JsonObject elem, Map mapModelJsons)
    {
        ...
        if (s != null)
        {
            ...
            if (jsonobject == null)
            {
                OFConfig.warn("BaseID not found: " + s);
            }
            else
            {
    ...
    private static void processExternalModel(JsonObject elem, Map mapModelJsons, String basePath)
    {
        ...
        if (s != null)
        {
            ...
            try
            {
                ...
                if (jsonobject == null)
                {
                    OFConfig.warn("Model not found: " + resourcelocation);
                    return;
                }
    ...
    private static void processId(JsonObject elem, Map mapModelJsons)
    {
        ...
        if (s != null)
        {
            if (s.length() < 1)
            {
                OFConfig.warn("Empty model ID: " + s);
            }
            else if (mapModelJsons.containsKey(s))
            {
                OFConfig.warn("Duplicate model ID: " + s);
            }
            else
            {
    ...
    {
        InputStream inputstream = OFConfig.getResourceStream(location);
        ...
        {
            String s = OFConfig.readInputStream(inputstream, "ASCII");
            inputstream.close();
